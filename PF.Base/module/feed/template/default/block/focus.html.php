...
<div class="activity_feed_content_text{if isset($aFeed.comment_type_id) && $aFeed.comment_type_id == 'poll'} js_parent_module_feed_{$aFeed.comment_type_id}{/if}{if !empty($aFeed.custom_class)} {$aFeed.custom_class}{/if}{if !empty($aFeed.status_background)} p-statusbg-feed" style="background-image: url('{$aFeed.status_background}');"{else}"{/if}>
        ...
        {if isset($aFeed.feed_status) && (!empty($aFeed.feed_status) || $aFeed.feed_status == '0')}
            <div class="activity_feed_content_status">{if strpos($aFeed.feed_status, '<br />') >= 200}{$aFeed.feed_status|feed_strip|shorten:200:'feed.view_more':true|split:55}{else}{$aFeed.feed_status|feed_strip|split:55|shorten:200:'feed.view_more':true}
                {/if}
            </div>
        {/if}

        {plugin call='feed.template_block_feed_focus'}
                ...
                {else}
                    <div class="feed_block_title_content {if (!empty($aFeed.feed_content) || !empty($aFeed.feed_custom_html)) && empty($aFeed.feed_image) && empty($aFeed.feed_image_banner)} activity_feed_content_no_image{/if}{if !empty($aFeed.feed_image)} activity_feed_content_float{/if}"{if isset($aFeed.feed_custom_width)} style="margin-left:{$aFeed.feed_custom_width};"{/if}>
                        {if !empty($aFeed.feed_title) || $aFeed.type_id == 'link'}
                            ...
                            {/if}
                            <a href="{if isset($aFeed.feed_link_actual)}{$aFeed.feed_link_actual}{else}{$aFeed.feed_link}{/if}" class="activity_feed_content_link_title{if isset($aFeed.custom_css)} {$aFeed.custom_css}{/if}"{if isset($aFeed.is_external_url)} target="_blank"{/if}{if isset($aFeed.is_external_url) && Phpfox::getParam('core.no_follow_on_external_links')} rel="nofollow"{/if}>{$aFeed.feed_title|clean|split:30}</a>
                            {if isset($aFeed.feed_title_sub)}
                                </span>
                            {/if}
                            {if !empty($aFeed.feed_title_extra)}
                            <div class="activity_feed_content_link_title_link">
                                <a href="{$aFeed.feed_title_extra_link}" class="{if isset($aFeed.custom_css)}{$aFeed.custom_css}{/if}" {if isset($aFeed.is_external_url)} target="_blank"{/if}{if isset($aFeed.is_external_url) && Phpfox::getParam('core.no_follow_on_external_links')} rel="nofollow"{/if}>{$aFeed.feed_title_extra|clean}</a>
                            </div>
                            {/if}
                        {/if}
                        {if !empty($aFeed.feed_content)}
                            <div class="activity_feed_content_display">
                                {$aFeed.feed_content|feed_strip|split:55}
                            </div>
                        {/if}
    ...
    {if Phpfox::getParam('feed.enable_check_in') && Phpfox::getParam('core.google_api_key') != '' && !empty($aFeed.location_name) && isset($aFeed.location_latlng) && isset($aFeed.location_latlng.latitude) && $aFeed.type_id != 'photo' && $aFeed.type_id != 'v' && $aFeed.type_id != 'link' && empty($aFeed.status_background)}
