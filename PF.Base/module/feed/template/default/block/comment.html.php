...
<a href="{url link='feed.comments'}?type={$aFeed.type_id}&id={$aFeed.item_id}&page={$nextIteration}{if defined('PHPFOX_FEED_STREAM_MODE')}&stream-mode=1{/if}" class="load_more_comments ajax"  onclick="$(this).addClass('active');"><i class="fa fa-spin fa-circle-o-notch"></i><span>{_p var='view_previous_comments'}</span></a>
...
<div id="js_feed_like_holder_{if isset($aFeed.like_type_id) && !isset($aFeed.is_app)}{$aFeed.like_type_id}{else}{$aFeed.type_id}{/if}_{if isset($aFeed.like_item_id) && !isset($aFeed.is_app)}{$aFeed.like_item_id}{else}{$aFeed.item_id}{/if}" class="comment_mini_content_holder{if (isset($aFeed.is_app) && $aFeed.is_app && isset($aFeed.app_object))} _is_app{/if}"{if (isset($aFeed.is_app) && $aFeed.is_app && isset($aFeed.app_object))} data-app-id="{$aFeed.app_object}"{/if}>
        <div class="comment_mini_content_holder_icon"{if isset($aFeed.marks) || (isset($aFeed.likes) && is_array($aFeed.likes)) || (isset($aFeed.total_comment) && $aFeed.total_comment > 0)}{else}{/if}></div>
			<div class="comment_mini_content_border">
                ...
                <div class="feed_options_holder item_options_holder hide" data-component="feed-options">
				...
				<div class="comment_mini_content_commands">
					{if isset($aFeed.like_type_id) && !(isset($aFeed.disable_like_function) && $aFeed.disable_like_function) && !Phpfox::getService('user.block')->isBlocked(null, $aFeed.user_id)}
					...
					{if Phpfox::isModule('share') && !isset($aFeed.no_share) && !empty($aFeed.type_id) && (!isset($bGroupIsShareable) || $bGroupIsShareable)  && !Phpfox::getService('user.block')->isBlocked(null, $aFeed.user_id)}
        ...
        <div id="js_feed_comment_post_{$aFeed.feed_id}" class="js_feed_comment_view_more_holder">
            ...
            {if Phpfox::isAppActive('Core_Captcha') && Phpfox::getUserParam('captcha.captcha_on_comment')}
