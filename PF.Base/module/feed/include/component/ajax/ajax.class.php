...
class Feed_Component_Ajax_Ajax extends Phpfox_Ajax
{
    ...
    public function updatePost()
    {
        ...
        if (isset($aVals['feed_id']) && Phpfox::getService('feed.process')->updateFeedComment($aVals['feed_id'], $aVals['user_status'], !empty($aVals['tagged_friends']) ? explode(',', $aVals['tagged_friends']) : [], isset($aVals['location']) ? $aVals['location'] : [])
        ) {
            Phpfox::getService('feed')->processUpdateAjax($aVals['feed_id']);
            $this->call('tb_remove();');
