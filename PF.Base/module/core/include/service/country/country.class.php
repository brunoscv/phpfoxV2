...
class Core_Service_Country_Country extends Phpfox_Service 
{
	...
	{	
		...
		if (false === ($this->_aCountries = $this->cache()->get($sCachedId)))
		{
			...
			{
				$this->_aCountries[$aRow['country_iso']] = (Core\Lib::phrase()->isPhrase('core.translate_country_iso_' . strtolower($aRow['country_iso'])) ? _p('translate_country_iso_' . strtolower($aRow['country_iso'])) : $aRow['name']);
			}					
    ...
    public function getChild($iChildId)
	{
		...
		if ($bIsChecked === false)
		{
			...
			if (false === ($this->_aChildren = $this->cache()->get($sCacheId)))
			{
	...
	public function getCountriesAndChildren()
	{
		...
		if (false === ($aCountries = $this->cache()->get($sCacheId)))
		{
	...
	public function getChildren($sCountry)
	{
		...
		if (false === ($aChildrenData = $this->cache()->get($sCacheId)))
		{
        ...
        $iActive = db()->select('is_active')
            ->from(Phpfox::getT('country'))
            ->where('country_iso = "' . $sCountry . '"')
            ->execute('getSlaveField');
        
        if (!is_array($aChildrenData) || empty($iActive)) {
            $aChildrenData = [];
        }
