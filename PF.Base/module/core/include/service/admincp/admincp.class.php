...
class Core_Service_Admincp_Admincp extends Phpfox_Service
{
    ...
    public function getNews($bSlide = false)
    {
        ...
        if (false === ($aCache = $this->cache()->get($bSlide ? $sSlideCacheId : $sMoreCacheId, 60))) {
            try {
                $aNews = Phpfox::getLib('xml.parser')->parse(Phpfox_Request::instance()->send('http://feeds.feedburner.com/phpfox', [], 'GET'));
    ...
    private function _processNews($aItem)
    {
        ...
        return [
            'title' => $aItem['title'],
            'link' => $aItem['link'],
            'description' => utf8_encode($aItem['description']),
            'image' => isset($aLinkInfo['default_image']) ? $aLinkInfo['default_image'] : '',
