...
class Attachment_Service_Attachment extends Phpfox_Service
{
    ...
    public function get($aConds, $sSort = 'attachment.time_stamp DESC', $bCount = true)
    {
        ...
        foreach ($aItems as $iKey => &$aItem) {
            $aItem['url'] = sprintf($aItem['destination'], '');
    ...
    public function getForItemEdit($iItemId, $sCategory, $iUserId = null, $bGetArray = false)
    {
        ...
        $aRows = $this->database()->select('attachment_id, destination')
            ->from($this->_sTable)
     ...
     * @return array
     */
    public function getForDownload($iId, $isUrl = false)
    {
        ...
        if (false === ($aRow = $this->cache()->get($sCacheId)) || $isUrl) {
            if ($isUrl) {
                $url = preg_replace('/\.[^.]+$/', '', $iId);
                $where = 'attachment.destination like \'' . $url . '%\'';
            } else {
    ...
    public function getCountForItem($iItemId, $sCategory, $bNoCache = false)
    {
        ...
        if ($bNoCache || (false === $iItemCount = $this->cache()->get($sCacheId))) {
    ...
    public function getTotal($iUserId = null)
    {
        ...
        if (false === ($iTotal = $this->cache()->get($sCacheId))) {
    ...
    public function hasAccess($iId, $sUserPerm, $sGlobalPerm)
    {
        ...
        if (false === ($aRow = $this->cache()->get($sRowCacheId))) {
