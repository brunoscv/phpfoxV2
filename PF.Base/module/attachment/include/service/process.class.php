 ...
 * @version        $Id: process.class.php 5840 2013-05-09 06:14:35Z phpFox LLC $
 */
class Attachment_Service_Process extends Phpfox_Service
{
    ...
    public function updateItemId($sId, $iUserId, $iItemId)
    {
        ...
        foreach ($aIds as $iId) {
            ...
            $aAttachment = $this->database()->select('*')
                ...
                ->execute('getSlaveRow');
            if ($aAttachment) {
                $this->database()->update(Phpfox::getT('attachment'), ['item_id' => $iItemId], "attachment_id = " . $iId . " AND user_id = " . $iUserId . "");
    ...
    {
        $this->database()->update(Phpfox::getT('attachment'), ['description' => Phpfox::getLib('parse.input')->clean($sDescription, 255)], "attachment_id = " . $iId . " AND user_id = " . $iUserId . "");
    ...
    public function updateInline($iId, $bRemove = false)
    {
        ...
        $this->database()->update($this->_sTable, ['is_inline' => ($bRemove ? 0 : 1)], 'attachment_id = ' . (int)$iId . ' AND user_id = ' . Phpfox::getUserId());
    ...
    public function deleteForItem($iUserId = null, $iItemId, $sCategory)
    {
        ...
        foreach ($aRows as $aRow) {
            ...
            $aFilesToDelete = [$sThumbnail, $sViewImage, $sActualImage];
            ...
            foreach ($aFilesToDelete as $sFilePath) {
                if ($aRow['server_id'] > 0) {
                    ...
                    if (file_exists($sFilePath)) {
                        $aFileSizes[$aRow['user_id']] += filesize($sFilePath);
                    }
                }

                // remove file from anywhere
    ...
    public function delete($iUserId, $iId)
    {
        ...
        if ($aRow) {
            if (!empty($aRow['destination'])) {
                $iFileSizes = 0;
                $dirAttachment = Phpfox::getParam('core.dir_attachment');
                $destination = $aRow['destination'];
                $aFilesToDelete = [$dirAttachment . sprintf($destination, '_thumb'), $dirAttachment . sprintf($destination, '_view'), $dirAttachment . sprintf($destination, '')];
                foreach ($aFilesToDelete as $sFilePath) {
                    if ($aRow['server_id'] > 0) {
                        // Get the file size stored when the photo was uploaded
                        $sTempUrl = Phpfox::getLib('cdn')->getUrl(str_replace(Phpfox::getParam('core.dir_attachment'), Phpfox::getParam('core.url_attachment'), $sFilePath));
    ...
    public function updateActivity($iId, $iType)
    {
        ...
        $this->database()->update(Phpfox::getT('attachment_type'), ['is_active' => (int)($iType == '1' ? 1 : 0)], 'extension = \'' . $iId . '\'');
