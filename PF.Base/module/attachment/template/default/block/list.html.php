		...
		<div class="attachment-row" id="js_attachment_id_{$aAttachment.attachment_id}">
            <div id="js_attachment_id_content_{$aAttachment.attachment_id}" class="item-outer">
                <div class="attachment-image {if !$aAttachment.is_image && !$aAttachment.is_video}attachment-image-icon{/if}">
                    {if $aAttachment.is_image && $aAttachment.destination}
                    ...
                    {elseif $aAttachment.is_video && $aAttachment.video_image_destination}
                ...
                <div class="attachment-body">
                    ...
                    <div class="attachment-row-title">
                        ...
                        {else}
                        <a href="{url link='attachment.download' url=''$aAttachment.url''}" class="attachment_row_link no_ajax_link item-title">{$aAttachment.file_name}</a> <span class="item-info">{$aAttachment.file_size|filesize} . <span id="attachment_counter_{$aAttachment.attachment_id}">{$aAttachment.counter}</span> {_p var='views'}</span>
                        {/if}
                    </div>
                    <div class="attachment-row-actions">
                        {if $bIsAttachmentNoHeader}
                        <span class="js_attachment_remove_inline" style="display: none;">
                            <a role="button" onclick="$Core.Attachment.removeInline(this, {if $aAttachment.inline.path}'{$aAttachment.inline.path}'{else}{$aAttachment.attachment_id}{/if}, '{$sEditorId}')" data-inline-path="{$aAttachment.inline.path}" class="js_hover_title">
                                <span class="ico ico-godown"></span>
                                <span class="js_hover_info">{_p var='remove_inline'}</span>
                            </a>
                        </span>
                        <span class="js_attachment_add_inline">
                            <a role="button" onclick="$Core.Attachment.insertInline(this, '{$aAttachment.inline.name}', '{$aAttachment.attachment_id}{if $aAttachment.is_image}:view{/if}', '{$aAttachment.inline.path}', '{$aAttachment.inline.url}', {if $aAttachment.is_image}true{else}false{/if}, '{$sEditorId}')" class="js_hover_title">
