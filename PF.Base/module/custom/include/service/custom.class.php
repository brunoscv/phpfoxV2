...
class Custom_Service_Custom extends Phpfox_Service
{
        ...
        switch ($aField['var_type']) {
            ...
            case 'radio':
                ...
                foreach ($aOptions as $aOption) {
                    $sContent .= '<option value="' . $aOption['option_id'] . '"' . (!empty($sUserValue['option_id']) && $sUserValue['option_id'] == $aOption['option_id'] ? ' selected="selected"' : '') . '>' . _p($aOption['phrase_var_name']) . '</option>';
                }
                $sContent .= '</select>';
    ...
    public function getForListing()
    {
        ...
        $aCustomFields = [];
        foreach ($aFields as $aField) {
            $aField['user_group_name'] = _p($aField['user_group_name']);
        ...
        foreach ($aGroups as $iKey => $aGroup) {
            $aGroups[$iKey]['user_group_name'] = _p($aGroup['user_group_name']);
