...
// class methods
    ...
    )
    {
        ...
        $apiUrl = self::API_URL;
		if($url == 'media/upload.json')
		{
			$apiUrl = "https://upload.twitter.com/1.1";
		}
		
         // calculate the base string
         $base = $this->calculateBaseString(
            $apiUrl . '/' . $url, $method, $data
         );
        ...
        $headers[] = $this->calculateHeader($oauth, $apiUrl . '/' . $url);
        $headers[] = 'Expect:';

        // set options
        $options[CURLOPT_URL] = $apiUrl . '/' . $url;
...
// Tweets resources
    ...
    public function statusesUpdateWithMedia($status, $media = null, $inReplyToStatusId = null, $lat = null, $long = null,
        $placeId = null, $displayCoordinates = null, $trimUser = null)
	{
        $parameters['media_data'] = $media;
		// make the call to upload media
        $respones = (array) $this->doCall(
            'media/upload.json',
            $parameters, true, 'POST'
        );
		
		unset($parameters['media_data']);
		$parameters['status'] = (string) $status;
		if (!empty($respones['media_id'])) 
		{
            $parameters['media_ids'] = $respones['media_id'];
        }
		if ($inReplyToStatusId != null) {
            $parameters['in_reply_to_status_id'] = (string) $inReplyToStatusId;
        }
...
// User resources
    ...
    public function accountVerifyCredentials(
        $includeEntities = null, $skipStatus = null, $includeEmail = null
    )
    {
        // build parameters
        $parameters = null;
        ...
        if ($includeEmail !== null) {
            $parameters['include_email'] = ($includeEmail) ? 'true' : 'false';
        }
        // make the call
        return (array) $this->doCall(
...
// Help resources
  ...
  /**
  * New function by YouNet Company
  */
