...
<div class="panel-default">
    <div class="panel-heading">
        <div class="panel-title" style="position: relative">
            {_p('Testimonials')}
            {if count($aPageItems) < 5}
            <button onclick="window.location.href='{url link='admincp.ynresphoenix.add'}{$sType}'" style="position: absolute; right: 0; bottom: -6px;" class="btn btn-success btn-sm">{_p('Add Testimonial')}</button>
            {/if}
        </div>
    </div>
</div>
    ...
    <div class="panel panel-default" id="ynresphoenix_manage_pages">
        ...
        <table id="js_drag_drop" cellpadding="0" cellspacing="0" class="table table-bordered">
            <thead>
                <tr>
                    ...
                    <th class="w20""></th>
                    <th class="w50">{_p('Icon')}</th>
                    <th class="w180">{_p('User')}</th>
                    <th>{_p('Content')}</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr class="checkRow{if is_int($iKey/2)} tr{else}{/if}">
                    ...
                    <td class="t_center">
                        ...
                        <div class="link_menu">
                            <ul>
                                ...
                                <li><a href="{url link='admincp.ynresphoenix.manage'}{$sType}?delete={$aItem.item_id}" class="sJsConfirm" data-message="{_p('Are you sure you want to delete this testimonial?')}">{_p('Delete')}</a></li>
                            </ul>
                        </div>
                    </td>
                    <td>
                    ...
                    <td>
                        {if isset($aItem.description) && Phpfox::isPhrase($this->_aVars['aItem']['description'])}
                            {phrase var=$aItem.description}
                        {else}
                            {$aItem.description|convert}
                        {/if}
                    </td>
        ...
        <div class="panel-footer">
            <div class="extra_info" style="float: left;">
                <span>{_p('You can have maximum 5 testimonials')}</span>
                <br/>
                <span>{_p('You can go to Manage Pages page')} ->  {if Phpfox::isPhrase($this->_aVars['aPage']['title'])}{_p var=$aPage.title}{else}{$aPage.title|convert}{/if} -> {_p('Settings to configure settings for this page')}</span>
            </div>
