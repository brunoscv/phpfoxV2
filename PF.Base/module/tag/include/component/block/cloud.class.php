...
class Tag_Component_Block_Cloud extends Phpfox_Component 
{
	...
	{		
		...
		$sTrendingSince = date(Phpfox::getParam('core.global_update_time'), $iSince);
		if (strpos(Phpfox::getParam('core.global_update_time'), 'F') !== false)
		{
			$sTrendingSince = str_replace(date('F', $iSince), Phpfox::getLib('date')->getMonth(date('n', $iSince)), $sTrendingSince);
		}
		else if (strpos(Phpfox::getParam('core.global_update_time'), 'M') !== false)
		{
