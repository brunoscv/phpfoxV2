...
class Ban_Service_Process extends Phpfox_Service
{
    ...
    public function add($aVals, &$aBanFilter = null)
    {
        ...
        if (isset($aVals['type_id']) && $aVals['type_id'] === 'email') {
            //allow wildcard email *@example.com
            if (substr($aVals['find_value'], 0, 2) !== '*@') {
                $aForm['find_value'] = [
                    'type' => 'regex:email',
        ...
        if ((isset($aVals['bShow']) && $aVals['bShow'] == '0') || !isset($aVals['bShow'])) {
            ...
            $aVals['reason'] = !Core\Lib::phrase()->isPhrase($aVals['reason']) ? Phpfox::getLib('parse.input')->clean($aVals['reason']) : $aVals['reason'];
        ...
        $this->cache()->remove('ban_' . $aVals['type_id']); // remove ban type
        $this->cache()->remove('ban_detail_check_' . $aVals['type_id']); // remove ban type
    ...
    {
        Phpfox::isAdmin(true);
        $row = $this->database()->select('type_id')
            ->from($this->_sTable)
            ->where('ban_id = ' . (int)$iDeleteId)
            ->execute('getSlaveRow');
        $this->database()->delete($this->_sTable, 'ban_id = ' . (int)$iDeleteId);
        if($row) {
            $this->cache()->remove('ban_' . $row['type_id']); // remove ban type
            $this->cache()->remove('ban_detail_check_' . $row['type_id']); // remove ban type
        }
        $this->cache()->removeGroup('ban');
        return true;
    }
