...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        ...
        <div class="form-group js_core_init_selectize_form_group">
            <label for="m_connection">{_p var='connection_page'}</label>
            <select name="val[m_connection]" id="m_connection" class="form-control">
                <option value="">{_p var='select'}:</option>
                <option value="site_wide" {if $bIsSiteWide}selected="true"{/if}>{_p var='site_wide'}</option>
                ...
                <optgroup label="{$sName|translate:'module'}">
                    ...
                    <option value="{$aCont.m_connection}"{value type='select' id='m_connection' default=$aCont.m_connection}>
                        {_p var='controller_'$aCont.m_connection} ({$aCont.m_connection})</option>
                    {/foreach}
                </optgroup>
                {/foreach}
            </select>
        ...
        {else}
        <div class="form-group js_core_init_selectize_form_group js_block_type js_block_type_id_0 {if $bIsEdit && $aForms.type_id > 0}hide{/if}">
        ...
        <div class="form-group hide custom-radio-wrapper">
            ...
            <label>
                <input type="radio" name="val[can_move]" value="1"{value type='radio' id='can_move' default='1'}/>
                <span class="custom-radio"></span>
                {_p var='yes'}
            </label>
            <label>
                <input type="radio" name="val[can_move]" value="0"{value type='radio' id='can_move' default='0' selected=true}/>
                <span class="custom-radio"></span>
                {_p var='no'}
            </label>
        </div>
