...
class User_Service_Cancellations_Cancellations extends Phpfox_Service
{
     ...
     * @return array
     */
	public function getFeedback($sOrder = '', $optionId = null, $aConditions = null)
	{
        $sOrder = !empty($sOrder) ? $sOrder : 'time_stamp DESC';
        $aWhere = !empty($aConditions) ? $aConditions : [];
        if($optionId){
            $aWhere[] = 'AND udf.reasons_given LIKE "%' . $optionId . '%"';
        }
