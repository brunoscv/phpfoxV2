...
$Behavior.user_inactivereminder_init = function()
{
	...
	$('#btnProcess').off('click').click(function() {
	...
	$("#btnStop").off('click').click(function() {
		$bKeepSending = false;
	...
	$("#btnSendAll").off('click').click(function() {
		var iDays = $('#inactive_days').val();
		if (parseInt(iDays) < 0) {
			return false;
		}
		$Core.jsConfirm({message: oTranslations['are_you_sure_you_want_send_mail_to_all_inactive_members_who_have_not_logged_in_for_days_days'].replace('{days}',iDays)}, function() {
			$.ajaxCall('user.addInactiveJob', 'all=1' + '&days=' + iDays);
		}, function(){});
