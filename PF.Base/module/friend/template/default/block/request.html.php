 ...
 * @version 		$Id: request.html.php 6551 2013-08-30 10:50:19Z phpFox LLC $
 */
...
{/if}
{if !empty($isFriendRequestAjax)}
<div>
    <input type="hidden" name="val[friend_request_ajax]" value="true" />
</div>
{/if}
<div>
...
{else}
	...
	<div class="p_top_15">
		<div class="table_clear" id="container_submit_friend_request">
			<input type="submit" onclick="$(this).attr('disabled', 'disabled');{if $bSuggestion}$('#js_friend_suggestion').hide(); $('#js_friend_suggestion_loader').show(); {/if}$('#js_process_image').show(); $('#js_process_request').ajaxCall('friend.addRequest');" value="{_p var='send_friend_request'}" class="button btn-primary" /> <span id="js_process_image" style="display:none;">{img theme='ajax/add.gif'}</span>
		</div>
	</div>
{/if}
</form>
