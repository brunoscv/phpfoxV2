...
class Friend_Service_Process extends Phpfox_Service
{
    ...
    public function add($iUserId, $iFriendId, $iFolderId = 0)
    {
        ...
        if ($friendshipDirection != 'one_way_friendships') {
            ...
            Phpfox::getLib('mail')->to($iFriendId)
                ->subject(['friend.full_name_confirmed_you_as_a_friend_on_site_title', ['full_name' => Phpfox::getUserBy('full_name'), 'site_title' => Phpfox::getParam('core.site_title')]])
                ->message(['friend.full_name_confirmed_you_as_a_friend_on_site_title_to_view_their_profile', ['full_name' => Phpfox::getUserBy('full_name'), 'site_title' => Phpfox::getParam('core.site_title'), 'link' => $sLink]])
    ...
    public function deny($iUserId, $iFriendId)
    {
        ...
        $this->database()->update(Phpfox::getT('friend_request'), ['is_ignore' => 1], 'user_id = ' . (int)$iUserId . ' AND friend_user_id = ' . (int)$iFriendId);
