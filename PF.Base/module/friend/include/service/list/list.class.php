 ...
 * @version 		$Id: list.class.php 5608 2013-04-03 11:52:10Z phpFox LLC $
 */
class Friend_Service_List_List extends Phpfox_Service 
{
	...
	/**
     * Get friends for list
     * @param $iListId
     * @return array|
     */
	public function getUsersByListId($iListId)
    {
        if(!$iListId)
        {
            return false;
        }
        return get_from_cache('friend_list_'.$iListId, function() use($iListId){
            $aFriends = db()->select('fld.friend_user_id as user_id')
                ->from($this->_sTable,'fl')
                ->join(Phpfox::getT('friend_list_data'),'fld','fld.list_id = fl.list_id')
                ->where('fl.list_id = '. (int)$iListId)
