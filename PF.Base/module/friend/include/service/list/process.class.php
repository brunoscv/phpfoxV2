 ...
 * @version 		$Id: process.class.php 5605 2013-04-02 09:32:58Z phpFox LLC $
 */
class Friend_Service_List_Process extends Phpfox_Service 
{
	...
	/**
     * Update friends for list
     * @param $iListId
     * @param $aMembers
     * @return bool
     */
	public function updateFriendListData($iListId, $aMembers)
    {
        if (!Phpfox::getService('friend.list')->getList($iListId, Phpfox::getUserId())) return false;

        db()->delete(Phpfox::getT('friend_list_data'),'list_id = '. (int)$iListId);

        foreach($aMembers as $iUserId)
        {
            db()->insert(Phpfox::getT('friend_list_data'), array(
                    'list_id' => (int) $iListId,
                    'friend_user_id' => (int) $iUserId
                )
            );
        }
        $this->cache()->remove('friend_list_'.$iListId);
        return true;
    }
