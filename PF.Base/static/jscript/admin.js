...
$(document).on('click', '#table_hover_action_holder input', function() {
  var th = $(this);

  if (typeof th.data('confirm-message') === 'string') {
    $Core.jsConfirm({
...
$Behavior.admincp_table_toggle_row = function(){
    if($('.js_admincp_table_header_toggle').length){
        $('.js_admincp_table_header_toggle').off('click').click(function () {
...
$Core.AdminCP.processUsers = {
    ...
    processImportUser: function(oObj){
        ...
        if(type == 'upload')
        {
            ...
            {
                $('#js_check_file_message').html('<div class="error-message">' + oTranslations['you_need_to_upload_file_first'] + '</div>');
            }
        }
        else if (type == 'start')
        {
...
$Behavior.admincp_process_country = function(){
    $('.js_active_multiple_item').on('click', function(){
    ...
    $('.js_active_item').on('click', function(){
    ...
    $('.js_active_multiple_item_btn').click(function(){
        var sIso = '';
