    ...
    public function processPurchase($aParams, $aPurchase)
    {
        if($aParams['status'] == "completed")
        {
            ...
            $aPoints = db()->select('a.activity_points, s.total_bought')
                                ...
                                ->join(Phpfox::getT('activitypoint_statistics'),'s','s.user_id = a.user_id')
                                ->where('s.user_id = '.(int)$aPurchase['user_id'])
                                ->execute('getSlaveRow');
            $iNew = (int)$aPoints['activity_points'] + (int)$aPurchase['points'];
            db()->update(Phpfox::getT('user_activity'),['activity_points' => $iNew], 'user_id = '.(int)$aPurchase['user_id']);
            $iNewBoughtPoint = (int)$aPoints['total_bought'] + (int)$aPurchase['points'];
            db()->update(Phpfox::getT('activitypoint_statistics'),['total_bought' => $iNewBoughtPoint], 'user_id = '.(int)$aPurchase['user_id']);
        }
    }
