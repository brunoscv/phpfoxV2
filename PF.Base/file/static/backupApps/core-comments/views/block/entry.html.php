			...
			<ul>
                ...
                {if !isset($bIsCommentAdminPanel)}
                    ...
                    {if (Phpfox::getUserParam('comment.edit_own_comment') && Phpfox::getUserId() == $aRow.user_id) || Phpfox::getUserParam('comment.edit_user_comment')}
                        <li><a href="#" onclick="return $Core.Comment.getEditComment({$aRow.comment_id});">{_p var='edit'}</a></li>
                    {/if}
                    {if Phpfox::getUserParam('comment.can_post_comments') && $bCanPostOnItem}
                        ...
                        {/if}
                    {/if}
                    {if ( (Phpfox::getUserParam('comment.delete_own_comment') && Phpfox::getUserId() == $aRow.user_id))	|| Phpfox::getUserParam('comment.delete_user_comment')}
