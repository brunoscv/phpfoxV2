...
class Ajax extends Phpfox_Ajax
{
    ...
    public function getText()
    {
        ...
        if (!isset($bHasPluginCall)) {
            ...
            $this->call('$(\'.js_comment_quick_edit_holder_' . $iCommentId . '\').find(\'.js_comment_textarea_edit:first\').focus();var offset = $(\'.js_comment_quick_edit_holder_' . $iCommentId . '\').find(\'.js_comment_textarea_edit:first\').val().length*2; $(\'.js_comment_quick_edit_holder_' . $iCommentId . '\').find(\'.js_comment_textarea_edit:first\')[0].setSelectionRange(offset,offset);');
        }
        $this->call('$Core.loadInit();');
    }
    ...
    public function loadAttachEmoticon()
    {
        ...
        Phpfox::getBlock('comment.emoticon', ['feed_id' => $iFeedId, 'parent_id' => $iParentId, 'edit_id' => $iEditId]);
        if ($iParentId) {
            $this->call('$(\'#js_comment_form_holder_' . $iParentId . '\').find(\'.js_comment_box .item-box-input\').after(\'' . $this->getContent() . '\');');
        } else if ($iFeedId) {
            $this->call('$(\'#js_item_feed_' . $iFeedId . '\').find(\'.js_feed_comment_form_holder .js_comment_box .item-box-input\').after(\'' . $this->getContent() . '\');');
        } else {
            $this->call('$(\'.js_comment_text_inner_' . $iEditId . '\').find(\'.js_edit_comment_holder .js_app_comment_feed_form .item-box-input\').after(\'' . $this->getContent() . '\');');
        }
        $this->call('$Core.loadInit();');
        return true;
    }
    ...
    public function loadAttachSticker()
    {
        ...
        if ($iParentId) {
            ...
            $this->call('$(\'.js_comment_quick_edit_holder_' . $iEditId . '\').find(\'.js_comment_group_icon\').addClass(\'open\').append(\'' . $this->getContent() . '\');');
        }
        $this->call('$Core.loadInit();');
        return true;
    }
