...
class Process extends Phpfox_Pages_Process
{
    ...
    public function add($aVals, $bIsApp = false)
    {
        ...
        Phpfox::getService('like.process')->add('groups', $iId, null, null);
            ...
            if (isset($aVals['emails']) && $aVals['emails']) {
                ...
                foreach ($aEmails as $sEmail) {
                    ...
                    $bSent = $oMail->to($sEmail)
                        ->subject(_p('full_name_invited_you_to_the_group_title',
                            [
                                'full_name' => Phpfox::getUserBy('full_name'),
                                'title' => $aPage['title']
                            ]
    ...
    private function processSetCoverPhoto($pageId, $photoId)
    {
        ...
        if ($photoId) {
            ...
            Phpfox::getService('groups.process')->updateCoverPhoto($photoId, $pageId);
