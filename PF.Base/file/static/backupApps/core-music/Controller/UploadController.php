        ...
        $this->template()->setTitle(($bIsEdit ? _p('editing_song') . ': ' . $aEditSong['title'] : _p('share_songs')))
            ...
            ->setBreadCrumb(($bIsEdit ? _p('editing_song') . ': ' . $aEditSong['title'] : _p('share_songs')),
                ($bIsEdit) ? $this->url()->makeUrl('music.upload',
                    ['id' => $iId]) : $this->url()->makeUrl('music.upload'), true)
            ->setPhrase([
                    'select_an_mp3',
            ...
            ->assign([
                    ...
                    'sJavaScriptEditLink' => ($bIsEdit ? "\$Core.jsConfirm({message: '" . _p('are_you_sure',
                            ['phpfox_squote' => true]) . "'}, function(){ $('#js_submit_upload_image').show(); $('#js_music_upload_image').show(); $('#js_music_current_image').remove(); $.ajaxCall('music.deleteSongImage', 'id={$aEditSong['song_id']}'); },function(){}); return false;" : ''),
