        ...
        <div class="item-comment mb-2">
            ...
            <span class="item-total-view">
                ...
                {if $aAlbum.total_view != 1}
                    {_p var='music_total_views' total=$aAlbum.total_view|number_format}
                {else}
                    {_p var='music_total_view' total=$aAlbum.total_view|number_format}
                {/if}
                </span>
                <span>
                {if $aAlbum.total_play != 1}
                    {_p var='music_total_plays' total=$aAlbum.total_play|number_format}
                {else}
                    {_p var='music_total_play' total=$aAlbum.total_play|number_format}
                {/if}
                </span>
            </span>
        </div>
    ...
    <div class="item-player music_player">
        ...
        <div class="item-sub-info mt-3">
            <div class="item-description item_view_content">
                ...
                {$aAlbum.text|highlight:'search'|parse|shorten:200:'feed.view_more':true|split:55}
            </div>
        </div>
