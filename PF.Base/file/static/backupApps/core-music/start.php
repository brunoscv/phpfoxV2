...
\Phpfox_Module::instance()
    ...
    ->addServiceNames([
        ...
        'music.playlist'         => Service\Playlist\Playlist::class,
        'music.playlist.process' => Service\Playlist\Process::class,
        'music.playlist.browse'  => Service\Playlist\Browse::class
    ])
    ->addTemplateDirs([
    ...
    ->addComponentNames('controller', [
        ...
        'music.playlist'        => Controller\PlaylistController::class,
        'music.browse.playlist' => Controller\Browse\PlaylistController::class,
        'music.view-playlist'   => Controller\ViewPlaylistController::class
    ])
    ...
    ->addComponentNames('block', [
        ...
        'music.other-playlist'  => Block\OtherPlaylistBlock::class,
        'music.user-playlist'   => Block\UserPlaylistBlock::class,
        'music.add-to-playlist' => Block\AddToPlaylistBlock::class,
        'music.playlist-feed'   => Block\PlaylistFeedBlock::class,
    ]);
group('/music', function () {
    ...
    route('/admincp/genre/order', function () {
        ...
        Phpfox::getLib('cache')->removeGroup('music_genre');
        return true;
    });
    route('/', 'music.index');
    ...
    route('/message', function () {
        ...
        $aUploadedSongs = json_decode($_REQUEST['uploaded_songs'], true);

        if (!empty($aUploadedSongs)) {
            $aUploadedSongs = array_keys($aUploadedSongs);
            $sUploadHash = md5(Phpfox::getTime('d/m/Y H:i:s') . uniqid());

            foreach ($aUploadedSongs as $iSongId) {
                Phpfox::getService('music.process')->publicSong($iSongId, $sUploadHash);
            }
        }
    ...
    route('/playlist/:id/*', 'music.view-playlist')->where([':id' => '([0-9]+)']);
    route('/playlist/*', 'music.playlist');
    route('/browse/playlist/*', 'music.browse.playlist');
...
$sDefaultPlaylistPhoto = flavor()->active->default_photo('music_default_playlist_photo', true);
if (!$sDefaultPlaylistPhoto) {
    $sDefaultPlaylistPhoto = setting('core.path_actual') . 'PF.Site/Apps/core-music/assets/image/nophoto_playlist.jpg';
}
...
Phpfox::getLib('setting')->setParam('music.default_playlist_photo', $sDefaultPlaylistPhoto);
