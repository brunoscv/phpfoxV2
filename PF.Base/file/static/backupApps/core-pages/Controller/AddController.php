    ...
    public function process()
    {
        ...
        if ($iEditId && !empty($aPage)) {
            ...
            $this->template()->assign([
                ...
                'aForms'        => $aPage,
                'aPageMenus' => Phpfox::getService('pages')->_getMenu($aPage, Phpfox::getService('groups')->getUrl($aPage['page_id'], $aPage['title'],
                    $aPage['vanity_url']), true)
            ]);
        }
        ...
        $this->template()->setTitle((($bIsEdit && !empty($aPage)) ? '' . _p('editing_page') . ': ' . $aPage['title'] : _p('add_new_page')))
            ...
            ->assign([
                    'aPermissions' => (isset($aPage) && isset($aPage['page_id']) ? Phpfox::getService('pages')->getPerms($aPage['page_id']) : []),
