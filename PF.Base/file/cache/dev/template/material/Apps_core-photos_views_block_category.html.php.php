...
?>
<?php if (isset ( $this->_aVars['aCategories'] )): ?>
    <?php
						Phpfox::getLib('template')->getBuiltFile('core.block.category');
						?>
<?php else: ?>
<?php if ($this->_aVars['bParent']): ?>
        <ul class="action">
            <li>
                <a href="<?php if ($this->_aVars['aCallback'] === null):  echo Phpfox::getLib('phpfox.url')->makeUrl('photo');  else:  echo Phpfox::getLib('phpfox.url')->makeUrl($this->_aVars['aCallback']['url_home']);  endif; ?>" class="js_photo_category"><?php echo _p('all_categories'); ?></a><?php endif; ?>
<?php echo $this->_aVars['sCategories']; ?>
<?php if ($this->_aVars['bParent']): ?></li>
            </ul>
<?php endif; ?>
<?php endif; ?>
