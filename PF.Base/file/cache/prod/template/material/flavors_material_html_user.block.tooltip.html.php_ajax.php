...
<?php /* Cached: September 11, 2020, 4:58 pm */ ?>
<?php 
...
<?php (($sPlugin = Phpfox_Plugin::get('user.template_block_tooltip_1')) ? eval($sPlugin) : false); ?>
        ...
        <div class="user_tooltip_info_up">
            <a href="<?php echo Phpfox::getLib('phpfox.url')->makeUrl($this->_aVars['aUser']['user_name']); ?>" class="user_tooltip_info_user">
...
<?php if (! empty ( $this->_aVars['iInfoCount'] )): ?>
                <li class="no-info">
<?php echo _p('no_information_found'); ?>
                </li>
<?php endif; ?>
            </ul>
...
<?php if ($this->_aVars['aUser']['is_friend'] && $this->_aVars['aUser']['is_friend'] === 2): ?>
            <div class="friend_request_sent">
                <span class="ico ico-arrow-right mr-1"></span><?php echo _p('friend_request_sent'); ?>
            </div>
<?php endif; ?>
        <ul>
<?php if (! $this->_aVars['bLoginAsPage'] && Phpfox ::isUser() && Phpfox ::isModule('friend') && Phpfox ::getUserParam('friend.can_add_friends') && empty ( $this->_aVars['aUser']['is_friend'] )): ?>
                <li><a class="btn btn-primary btn-sm btn-icon" href="#" onclick="$(this).closest('.js_user_tool_tip_holder').hide();return $Core.addAsFriend('<?php echo $this->_aVars['aUser']['user_id']; ?>');" title="<?php echo _p('add_to_friends'); ?>"><span class="ico ico-user1-plus-o"></span><?php echo _p('add_friends'); ?></a></li>
<?php endif; ?>
<?php if ($this->_aVars['bShowBDay']): ?>
                <li><a class="btn btn-default btn-sm btn-icon" href="<?php echo Phpfox::getLib('phpfox.url')->makeUrl($this->_aVars['aUser']['user_name']); ?>"><span class="ico ico-birthday-cake-alt mr-1"></span><?php echo _p('birthday_wishes'); ?></a></li>
...
<?php else: ?>
<?php if ($this->_aVars['bCanSendMessage']): ?>
                    <li><a class="btn btn-default btn-sm" href="#" onclick="$Core.composeMessage({user_id: <?php echo $this->_aVars['aUser']['user_id']; ?>});$(this).closest('.js_user_tool_tip_holder').hide(); return false;"><?php echo _p('send_message'); ?></a></li>
<?php endif; ?>
<?php endif; ?>
        </ul>
    </div>
