    ...
    public function display($aParams, $bIsLoop = false)
    {
        ...
        if (isset($aParams['user'])) {
            ...
            $aParams['title'] = ($bIsOnline ?
                _p('full_name_is_online', array(
                    'full_name' => Phpfox::getLib('parse.output')->shorten(Phpfox::getLib('parse.output')->clean($aParams['user'][$sSuffix . 'full_name']), 0)
                ))
                : Phpfox::getLib('parse.output')->shorten(Phpfox::getLib('parse.output')->clean($aParams['user'][$sSuffix . 'full_name']), 0));
        ...
        if (empty($aParams['file'])) {
            ...
            if (isset($aParams['path'])
                ...
                $name = Phpfox::getLib('parse.output')->clean(isset($aParams['user']) ? $aParams['user'][$sSuffix . 'full_name'] : (isset($aParams['title']) ? $aParams['title'] : ''));
