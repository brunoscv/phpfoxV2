 ...
 * @author            phpFox LLC
 * @package        Phpfox
 * @version        $Id: parser.class.php 2525 2011-04-13 18:03:20Z phpFox LLC $
 */
class Phpfox_Xml_Parser
{
    ...
    /**
     * @return Phpfox_Xml_Parser
     */
    public static function instance()
    {
        return Phpfox::getLib('xml.parser');
    }
    ...
    public function parse($mFile, $sEncoding = 'ISO-8859-1', $bEmptyData = true)
    {
        ...
        xml_set_character_data_handler($this->_oXml, [&$this, '_handleCdata']);
        xml_set_element_handler($this->_oXml, [&$this, '_handleElementStart'], [&$this, '_handleElementEnd']);
        ...
        if ($bError) {
		    if(defined('PHPFOX_DEBUG') && PHPFOX_DEBUG) {
                ...
                $errorMessage = 'Error Code: ' . $this->errorCode() . ' -- Error Message: ' . $this->errorString() . ' --  Line: ' . $this->errorLine();
                Phpfox::getLog('xml_parser.log')->error($errorMessage);
            }
    ...
    private function _addNode(&$aChildrens, $sName, $sValue)
    {
        if (!is_array($aChildrens) || !in_array($sName, array_keys($aChildrens))) {
            ...
            $aChildrens[$sName] = [$aChildrens[$sName]];
            $aChildrens[$sName][] = $sValue;
        }
    }
