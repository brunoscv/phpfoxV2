...
class Phpfox_Locale
{
	...
	public function __construct()
	{
		...
		if (!($this->_aLanguages = $oCache->get($sLangAllId)))
		{
            ...
            $oCache->group('locale', $sLangAllId);
            $oCache->group('language', $sLangAllId);
		}
        ...
        if (Phpfox::getParam('core.auto_detect_language')) {
            if (isset($_SESSION['lang'])) {
                ...
                $sLanguageId = isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])
                    ...
                    : Phpfox::getParam('core.default_lang_id');
            }
            if (isset($this->_aLanguages[$sLanguageId])) {
                return $sLanguageId;
            }
	...
	public function translate($sStr, $sPrefix = null)
	{
		...
		if ($sPrefix == 'module') {
		    $sModulePhrase = 'module_' . $sStr;
            if (Core\Lib::phrase()->isPhrase($sModulePhrase)) {
                return _p($sModulePhrase);
            }
			$sStr = ucwords($sStr);
		}		
