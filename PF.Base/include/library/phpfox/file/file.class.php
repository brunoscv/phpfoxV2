 ...
 * @author          phpFox LLC
 * @package         phpFox
 * @version         $Id: file.class.php 6862 2013-11-07 11:00:19Z phpFox LLC $
 */
class Phpfox_File
{
    /**
     ...
     * argument. As well as check if the files ize is allowed bassed on the 3rd argument
     ...
     * @param int $iMaxSize Max file size in bytes for the file being uploaded
     * @param bool $bCheckSpace
     * @param int $iUserId
     ...
     */
    public function load($sFormItem, $aSupported = [], $iMaxSize = null, $bCheckSpace = true, $iUserId = null)
    {
        ...
        if (Phpfox::isUser() && $Request->getHeader('X-File-Name')) {
            ...
            if (isset($_FILES['ajax_upload'])) {
                ...
                $file = PHPFOX_DIR_FILE . 'static' . PHPFOX_DS . uniqid() . '.' . \Phpfox_File::instance()->extension($Request->getHeader('X-File-Name'));
        ...
        if (in_array('jpg', $aSupported) && !in_array('jpeg', $aSupported)) {
            array_push($aSupported, 'jpeg');
        }

        if (in_array('mpg', $aSupported) && !in_array('mpeg', $aSupported)) {
        ...
        if ($bCheckSpace) {
            $iUserId = ($iUserId === null) ? Phpfox::getUserId() : $iUserId;
            if ($iUserId && !Phpfox::getService('user.space')->isAllowedToUpload($iUserId,
                    filesize($this->_aFile['tmp_name']))) {
                return false;
            }
        }

        if (count($aSupported) && !in_array(strtolower($this->_aFile['ext']), $aSupported)) {
            return Phpfox_Error::set(_p('not_a_valid_file_extension_we_only_accept_support',
                ['support' => implode(', ', $aSupported)]));
        }

        if (Phpfox_Image::instance()->isImageExtension($this->_aFile['ext']) && !Phpfox_Image::instance()->isImage($this->_aFile['tmp_name'])) {
            return Phpfox_Error::set(_p('not_a_valid_image_we_only_accept_the_following_file_extensions_support',
                ['support' => implode(', ', $aSupported)]));
        }
    ...
    )
    {
        ...
        if ($sPlugin = Phpfox_Plugin::get('library_phpfox_file_file_upload_1')) {
            eval($sPlugin);
        ...
        if (Phpfox::getParam(['balancer', 'enabled'])) {
        ...
        if (($sFormItem != "file") && file_exists($sFormItem)) {
            $this->_sExt = pathinfo($sFormItem, PATHINFO_EXTENSION);
        }
        $sDest = $this->_sDestination . $sFileName . '.' . $this->_sExt;

        if ($sPlugin = Phpfox_Plugin::get('library_phpfox_file_file_upload_2')) {
            eval($sPlugin);
            if (isset($mReturnFromPlugin)) {
                return $mReturnFromPlugin;
            }
        ...
        if (($sFormItem != "file") && file_exists($sFormItem)) {
            @copy($sFormItem, $sDest);
        } else {
            if (defined('PHPFOX_APP_USER_ID') || defined('PHPFOX_HTML5_PHOTO_UPLOAD') || Phpfox_Request::instance()->method() == 'PUT') {
                @copy($this->_aFile['tmp_name'], $sDest);
                if (!defined('PHPFOX_FILE_DONT_UNLINK')) {
                    @unlink($this->_aFile['tmp_name']);
                }
            } else {
                if (!@move_uploaded_file($this->_aFile['tmp_name'], $sDest)) {
                    return Phpfox_Error::set(_p('unable_to_move_the_file'));
                }
            }
        }
        if ($sPlugin = Phpfox_Plugin::get('library_phpfox_file_file_upload_3')) {
            eval($sPlugin);
            if (isset($mReturnFromPlugin)) {
                return $mReturnFromPlugin;
            }
        ...
        if (Phpfox_Image::instance()->isImageExtension($this->_sExt) && Phpfox_Image::instance()->isImage($sDest)) {
            Phpfox_Image::instance()->fixOrientation($sDest);
        }

        if ($bCdn === true) {
            $bReturn = Phpfox::getLib('cdn')->put($sDestination . str_replace('\\', '/',
                    str_replace($sDestination, '', $this->_sDestination) . $sFileName . '.' . $this->_sExt));
        ...
        return str_replace('\\', '/', str_replace($sDestination, '',
                $this->_sDestination) . $sFileName . (($bModifyFileName === true || is_array($bModifyFileName)) ? '%s.' : '.') . $this->_sExt);
    }
     ...
     */
    public function getAllFiles($sDir, $bRecurse = true)
    {
        //Check is valid dir
        if (!is_dir($sDir)) {
            return false;
        ...
        foreach (new DirectoryIterator($sDir) as $fileInfo) {
            if (!$fileInfo->isDot()) {
                ...
                $sNewDir = realpath($sDir . PHPFOX_DS . $fileName);
                if (is_dir($sNewDir) && $bRecurse) {
                    $this->getAllFiles($fileInfo->getPathname());
    ...
    public static function filesize($iSize, $iPrecision = 2)
    {
        ...
        if ($iSize >= 1099511627776) {
            $fSize = $iSize / 1099511627776;
            $sSuffix = 'tb';
        } elseif ($iSize >= 1073741824 && $iSize < 1099511627776) {
    ...
    public function forceDownload($sFile, $sName, $sMimeType = '', $sFileSize = '', $iServerId = 0)
    {
        ...
        if (!file_exists($sFile)) {
            $sFile = Phpfox::getLib('cdn')->getUrl(str_replace(PHPFOX_DIR, Phpfox::getParam('core.path_file'), $sFile),
                $iServerId);
        }
     ...
     *
     * @see copy()
     * @param string $sSrc Full path to the original file
     * @param string $sDest Full path to where the original file will be copied to
    ...
    {
        try {
            $return = Phpfox::getLib('cdn')->remove($sSrc);
        } catch (Exception $e) {
            $return = false;
        }
    ...
    /**
     * Get the temporary directory of the server based on the servers environment variables.
     ...
     *
     * @param int $iMaxSize You can define what you want as a limit (in MB).
     * @param bool $bReturnNumber
     * @return string Returns the actual limit the server has and if your limit passes the test it will just return your limit.
     */
    public function getLimit($iMaxSize, $bReturnNumber = false)
    {
        $aLimitSize = [
            $iMaxSize,
            $this->getSizeToMb(ini_get('upload_max_filesize')),
            $this->getSizeToMb(ini_get('post_max_size'))
        ];

        $aLimitSize = array_diff($aLimitSize, [0]);

        $iReturn = min($aLimitSize);
    ...
    public function getSizeToMb($val = '')
    {
        $val = trim($val);
    ...
    public function mkdir($sDir, $bRecurse = false, $mChmod = null)
    {
        ...
        if ($bRecurse === true) {
            ...
            foreach ($aParts as $sSubDir) {
                if (!is_dir($sParentDirectory . $sSubDir)) {
                    mkdir($sParentDirectory . $sSubDir);
                    //Create empty file index.html
                    $file = realpath($sParentDirectory . $sSubDir . PHPFOX_DS . 'index.html');
                    if (!file_exists($file)) {
                        touch($file);
                    }
                    if ($mChmod !== null) {
                        chmod($sParentDirectory . $sSubDir, 0777);
                    }
                }

                $sParentDirectory .= $sSubDir . PHPFOX_DS;
            }
        } else {
            if (!is_dir($sDir)) {
                mkdir($sDir);
            ...
            //Create empty file index.html
            $file = realpath($sDir . PHPFOX_DS . 'index.html');
            if (!file_exists($file)) {
                touch($file);
            }
            if ($mChmod !== null) {
    ...
    private function _passLimit()
    {
        ...
        */
        /**
         * TODO $iUploadMaxFileSize un-defined, so I define it is 0
         */
        $iUploadMaxFileSize = 0;
        switch ($this->_aFile['error']) {
            case 1:
                return Phpfox_Error::set(_p('upload_failed_server_cannot_handle_files_larger_then_file_size',
            ...
            default:
                ...
                switch (strtolower(substr($sPostMaxSize, -1))) {
                    case 'k' :
                        $iPostMaxSize = ((int)ini_get('post_max_size') * 1024);
                        break;
                    case 'm' :
                        $iPostMaxSize = ((int)ini_get('post_max_size') * 1048576);
                        break;
                    case 'g' :
                        $iPostMaxSize = ((int)ini_get('post_max_size') * 1073741824);
                        break;
                    default:
                        $iPostMaxSize = ((int)ini_get('post_max_size') * 1048576);
                        break;
                ...
                if ($iSize >= $iPostMaxSize) {
                    return Phpfox_Error::set(_p('upload_failed_server_cannot_handle_files_size_larger_then_file_size',
                        [
                            'size' => $this->filesize($iSize, 0),
                            'file_size' => $this->filesize($iUploadMaxFileSize)
                        ]));
                }

                if ($this->_iMaxSize !== null && $iSize >= ($this->_iMaxSize * 1048576)) {
                    return Phpfox_Error::set(_p('upload_failed_your_file_size_is_larger_then_our_limit_file_size',
                        [
                            'size' => $this->filesize($iSize, 0),
                            'file_size' => $this->filesize(($this->_iMaxSize * 1048576))
                        ]));
                }
     ...
     * by default the month/year we are in. This is a setting in the AdminCP
     ...
     * or $_FORM['FOO'][] with the latter being a multiple upload routine.
     *
     ...
     * @param string $sPath
     * @param string $sThumbnail
     * @param int $iServerId
     *
     * @return string
     */
    public function getPath($sDestination, $sPath, $sThumbnail = '', $iServerId = 0)
    {
        $sPath = Phpfox::getParam($sPath) . sprintf($sDestination, $sThumbnail);
        if ($iServerId) {
            $sRelativePath = str_replace(PHPFOX_DIR, '', $sPath);
            $sUrl = Phpfox::getLib('cdn')->getUrl($sRelativePath, $iServerId);
            file_put_contents($sPath, fox_get_contents($sUrl));
