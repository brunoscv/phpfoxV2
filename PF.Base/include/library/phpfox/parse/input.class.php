...
class Phpfox_Parse_Input
{
    ...
    public function allowTitle($sTitle, $sErrorMessage)
    {
        ...
        if (Phpfox::isAppActive('Core_Pages') && Phpfox::getService('pages')->isPage($sTitle)) {
            $bIsOk = false;
        }
    ...
    public function prepare($sTxt, $bNoClean = true, $extra = [])
    {
        ...
        if (isset($extra['comment'])) {
            $mentions = Phpfox::getService('user.process')->getIdFromMentions($sTxt, true);
            $oldComment = Phpfox_Database::instance()->select('text')->from(':comment_text')->where("comment_id=$extra[comment]")->executeField();
            ...
            foreach ($mentions as $userId) {
                Phpfox::getLib('mail')->to($userId)
                    ->subject(['user_name_tagged_you_in_a_comment', ['user_name' => $sTagger]])
                    ->message(['user_name_tagged_you_in_a_comment_check_it_out', ['user_name' => $sTagger, 'link' => $link]])
                    ->send();
            }
        }
    ...
    {
        $sTxt = str_replace("\n", "[pf_break]", $sTxt);
        $sTxt = preg_replace_callback('/\{php\}(.*?)\{\/php\}/is', [&$this, '_addBreak'], $sTxt);
        $sTxt = preg_replace_callback('/<(.*?)>/is', [&$this, '_addBreak'], $sTxt);
    ...
    public function stripInnerHtml($sText)
    {
        ...
        preg_replace('/<(.*?)>/is', "'<'.\$this->_stripInnerHtml('\\1').'>'", $sText);
    ...
    private function _cleanHtml($sTxt)
    {
        ...
        foreach ($aMatched[0] as $iKey => $sMatch) {
            ...
            if (strpos($sAllowed, $sTemp) !== false) {
                ...
                $sNew = str_replace(['<', '>'], [$iUnid1, $iUnid2], $sMatch2);
        ...
        $sTxt = str_replace(['<', '>'], ['&lt;', '&gt;'], $sTxt);
        $sTxt = str_replace([$iUnid1, $iUnid2], ['<', '>'], $sTxt);
    ...
    {
        $sAllowedHtml = '<p><br><br /><strong><em><u><ul><li><font><ol><img><div><span><blockquote><strike><sub><sup><h1><h2><h3><h4><h5><h6><a><b><i><hr><tt><s><center><big><abbr><pre><small><object><embed><param><code>';

        (($sPlugin = Phpfox_Plugin::get('parse_input__getallowedhtml')) ? eval($sPlugin) : null);
    ...
    private function _removeEvilAttributes($sTxt, $bCleanOnly = false)
    {
        ...
        switch (strtolower($aParts[0])) {
            ...
            case 'img':
                ...
                if (isset($aParams['src'])) {
                    $sTempSource = str_replace(['"', "'"], '', $aParams['src']);
                }
        ...
        $aHtmlTags = $this->getAllowedHtml();

        (($sPlugin = Phpfox_Plugin::get('parse_input__removeevilattributes')) ? eval($sPlugin) : null);

        if (is_array($aHtmlTags) && (isset($aHtmlTags[strtolower($aParts[0])]) || isset($aHtmlTags[substr_replace(strtolower($aParts[0]), '', 0, 1)])) && $bFailed === false) {
            return '<' . $sTxt . '>';
        } else {
            return '&lt;' . $sTxt . '&gt;';
        }
