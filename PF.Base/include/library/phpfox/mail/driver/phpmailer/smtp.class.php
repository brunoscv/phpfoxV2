 ...
 * @author          phpFox LLC
 * @package         Phpfox
 * @version         $Id: smtp.class.php 1666 2010-07-07 08:17:00Z phpFox LLC $
 */
class Phpfox_Mail_Driver_Phpmailer_Smtp implements Phpfox_Mail_Interface
{
    ...
    public function __construct()
    {
        ...
        if (!Phpfox::getParam('core.check_certificate_smtp_host_name')) {
            $this->_oMail->SMTPOptions = [
                'ssl' => [
                    "verify_peer" => false,
    ...
    public function test($aVals)
    {
        ...
        if (!empty($aVals['mail_smtp_secure']['value']) && $aVals['mail_smtp_secure']['value'] !== 'none') {
            $this->_oMail->SMTPSecure = $aVals['mail_smtp_secure']['value'];
        }

        if (!$aVals['check_certificate_smtp_host_name']) {
            $this->_oMail->SMTPOptions = [
                'ssl' => [
                    "verify_peer" => false,
                    "verify_peer_name" => false,
