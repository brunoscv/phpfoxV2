    ...
    public function process()
    {
        ...
        if (($iEditId = $this->request()->getInt('id')) || ($iEditId = $this->request()->getInt('album_edit_id'))) {
            if (($aAlbum = Phpfox::getService('music.album')->getForEdit($iEditId))) {
                ...
                $bIsEdit = true;
                $sModule = !empty($aAlbum['module_id']) ? $aAlbum['module_id'] : false;
                $iItem = $aAlbum['item_id'];
        ...
        if ($sModule !== false && $iItem !== false && Phpfox::hasCallback($sModule, 'getMusicDetails')) {
            if (($aCallback = Phpfox::callback($sModule . '.getMusicDetails', ['item_id' => $iItem]))) {
                ...
                if ($bIsEdit) {
                    ...
                    if ($iId = \Phpfox::getService('music.album.process')->add($aVals)) {
                        if (Phpfox::getUserParam('music.can_upload_music_public')) {
                            $this->url()->send('music.album.track.setup', ['id' => $iId],
                                _p('album_successfully_added'));

                        } else {
                            $this->url()->send('music.album.manage', ['id' => $iId],
                                _p('album_successfully_added'));
                        }
                    }
                }
        ...
        $this->template()->setTitle(($bIsEdit ? _p('editing_album') . ': ' . $aAlbum['name'] : _p('create_album')))
            ...
            ->assign([
                    ...
                    'sJavaScriptEditLink' => ($bIsEdit ? "\$Core.jsConfirm({message: '" . _p('are_you_sure',
                            ['phpfox_squote' => true]) . "'}, function(){ $('#js_submit_upload_image').show(); $('#music-dropzone').show(); $('#js_music_current_image').remove();$('#image_path').val('');$.ajaxCall('music.deleteImage', 'id={$aAlbum['album_id']}'); },function(){}); return false;" : ''),
                    'sMethodUrl'          => ($bIsEdit ? $this->url()->makeUrl('music.album.track',
                        ['id' => $aAlbum['album_id'], 'method' => 'simple']) : ''),
                    ...
                    'iPhotoMaxFileSize'   => Phpfox::isAppActive('Core_Photos') ? Phpfox::getUserParam('photo.photo_max_upload_size') : 0,
