...
class ViewAlbumController extends \Phpfox_Component
{
    ...
    public function process()
    {
        ...
        $this->setParam('aRatingCallback', [
                'type'           => 'music_album',
                'total_rating'   => _p('total_rating_ratings', ['total_rating' => $aAlbum['total_rating']]),
            ...
            $this->template()
                ->setHeader('cache', [
                    ...
                    '<script type="text/javascript">$Behavior.rateMusicAlbum = function() { $Core.rate.init({module: \'music_album\', display: ' . ($aAlbum['has_rated'] ? 'false' : ($aAlbum['user_id'] == Phpfox::getUserId() ? 'false' : 'true')) . ', error_message: \'' . ($aAlbum['has_rated'] ? _p('you_have_already_voted',
                        ['phpfox_squote' => true]) : _p('you_cannot_rate_your_own_album',
        ...
        $this->template()->setBreadCrumb(_p('music_albums'), ($aCallback === false ? $this->url()->makeUrl('music.browse.album') : $aCallback['url_home_photo']))
