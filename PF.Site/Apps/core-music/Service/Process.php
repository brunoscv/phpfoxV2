    ...
    public function publicSong($iSongId, $sUploadHash = null)
    {
        ...
        if (!Phpfox::getUserParam('music.music_song_approval')) {
            Phpfox::getService('user.space')->update(Phpfox::getUserId(), 'music',
                filesize(Phpfox::getParam('music.dir') . $aSong['song_path']));
        }
        ...
        if ((int)$aSong['album_id'] > 0) {
            if (!Phpfox::getUserParam('music.music_song_approval')) {
                ...
                if (!$bNewFeed) {
                    (Phpfox::isModule('feed') && Phpfox::getParam('music.music_allow_create_feed_when_add_new_item', 1) ? $_SESSION['music_song_' . $sUploadHash . '_' . $aSong['album_id']] = $iFeedId = \Phpfox::getService('feed.process')->callback($aCallback)->add('music_song',
                        $aSong['song_id'], $aSong['privacy'],
                        $aSong['privacy_comment'],
                        (!empty($aSong['item_id']) ? (int)$aSong['item_id'] : '0')) : null);
                }
            }
        } else {
            if (!Phpfox::getUserParam('music.music_song_approval')) {
    ...
    public function approve($iId)
    {
        ...
        if ($aSong['album_id']) {
            ...
            if ($aSong['module_id'] && $aSong['item_id'] && Phpfox::isModule($aSong['module_id']) && Phpfox::hasCallback($aSong['module_id'],
                ...
                (Phpfox::isModule('feed') && Phpfox::getParam('music.music_allow_create_feed_when_add_new_item', 1) ? \Phpfox::getService('feed.process')->callback(Phpfox::callback($aSong['module_id'] . '.getFeedDetails',
