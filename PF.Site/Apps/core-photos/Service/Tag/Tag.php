...
class Tag extends Phpfox_Service
{
     ...
     * @param      $aPhotoIds
     * @param null $iUserId
     * @param boolean $bOnFeed
     * @return array|int|string
     */
    public function getTagByIds($aPhotoIds, $iUserId = null, $bOnFeed = false)
    {
        ...
        $aExcludedUsers = [];
        if ($bOnFeed) {
            $aRemovedTag = db()->select('ftr.user_id')
                ->from(':feed_tag_remove', 'ftr')
                ->join(':user', 'u', 'u.user_id = ftr.user_id')
                ->where([
                    'ftr.type_id' => 'photo',
                    ' AND ftr.item_id IN (' . $sPhotoIds . ')'
                ])->executeRows();
            $aExcludedUsers = array_map(function ($item){
                return (int)$item['user_id'];
            }, $aRemovedTag);
        }
        return db()->select('pt.tag_user_id, ' . Phpfox::getUserField())
            ->from($this->_sTable, 'pt')
            ->join(':user', 'u', 'u.user_id = pt.tag_user_id')
            ->where('pt.photo_id IN (' . $sPhotoIds . ')' . (($iUserId != null) ? ' AND pt.tag_user_id != ' . (int)$iUserId : '') . (!empty($aExcludedUsers) ? (' AND pt.tag_user_id NOT IN (' . implode(',', $aExcludedUsers) . ')') : ''))
