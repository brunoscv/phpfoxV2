...
class Process extends Phpfox_Service
{
        ...
        if ((Phpfox::getUserParam('photo.can_tag_own_photo') && $aPhoto['user_id'] == Phpfox::getUserId()) || Phpfox::getUserParam('photo.can_tag_other_photos')) {
            ...
            if (!empty($aVals['tag_user_id'])) {
                ...
                if (isset($aUser['user_id'])) {
                    ...
                    if ($iIsUserTagged) {
                        return Phpfox_Error::set(_p('full_name_has_already_been_tagged_in_this_photo', ['full_name' => $aUser['full_name']]));
                    }
            ...
            if (Phpfox::isModule('feed')) {
                ...
                if ($iTagUserId != $aFeed['user_id']
                    && !Phpfox::getService('feed.tag')->checkTaggedUser($aVals['item_id'], 'photo', $iTagUserId)
                    && !Phpfox::getService('feed.tag')->isRemovedTag($iTagUserId, $aVals['item_id'], 'photo')) {
                    Phpfox::getService('feed.tag')->addTaggedUsers($aVals['item_id'], [$iTagUserId], 'photo');
                }
            }
            ...
            Phpfox::getLib('mail')->to($iTagUserId)
                ...
                ->message((Phpfox::getUserId() == $aPhoto['user_id'] ? [
                    'full_name_tagged_you_on_gender_photo', [
                        'full_name' => Phpfox::getUserBy('full_name'),
    ...
    public function delete($iId)
    {
        ...
        if (($aTag['post_user_id'] == Phpfox::getUserId() || $aTag['photo_owner_id'] == Phpfox::getUserId() || $aTag['tag_user_id'] == Phpfox::getUserId())) {
            ...
            if (Phpfox::isModule('feed')) {
                Phpfox::getService('feed.process')->delete('photo_tag', $aTag['tag_id']);
                Phpfox::getService('feed.process')->deleteTaggedUser($aTag['photo_id'], $aTag['tag_user_id'], 'photo');
            }
