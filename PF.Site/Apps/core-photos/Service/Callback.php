...
class Callback extends Phpfox_Service
{
    ...
    public function getToSponsorInfo($iId)
    {
        ...
        $aImage['paypal_msg'] = _p('sponsor_paypal_message_photo', ['sPhotoTitle' => $aImage['title']]);
        $aImage['image_dir'] = 'photo.url_photo';
        $aImage['title'] = _p('sponsor_title_photo', ['sPhotoTitle' => $aImage['title']]);
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        $aMessage = (Phpfox::getUserId() == $aRow['user_id'] ?
            [
                'full_name_commented_on_gender_photo_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                [
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'gender'    => Phpfox::getService('user')->gender($aRow['gender'], 1),
                    'link'      => $sLink,
            ...
            :
            [
                'full_name_commented_on_other_full_name_s_photo_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                [
                    'full_name'       => Phpfox::getUserBy('full_name'),
                    'other_full_name' => $aRow['full_name'],
                    'link'            => $sLink,
        ...
        Phpfox::getService('comment.process')->notify([
                ...
                'owner_subject'      => [
                    'full_name_commented_on_your_photo_title', [
                        'full_name' => Phpfox::getUserBy('full_name'),
                ...
                'owner_message'      =>
                    [
                        'full_name_commented_on_your_photo_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                        [
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'title'     => $aRow['title'],
                            'link'      => $sLink
    ...
    public function getNotificationLike($aNotification)
    {
        ...
        if ($aNotification['user_id'] == $aRow['user_id']) {
            $sPhrase = _p('user_name_liked_gender_own_photo_title', [
            ...
            $sPhrase = _p('user_name_liked_your_photo_title', [
                'user_name' => Phpfox::getService('notification')->getUsers($aNotification),
                'title'     => Phpfox::getLib('parse.output')->shorten($aRow['title'],
                    (Phpfox::isModule('notification') ? Phpfox::getParam('notification.total_notification_title_length') : $this->_iFallbackLength),
                    '...')
            ...
            $sPhrase = _p('user_name_liked_span_class_drop_data_user_full_name_s_span_photo_title', [
                'user_name' => Phpfox::getService('notification')->getUsers($aNotification),
    ...
    public function getCommentNotification($aNotification)
    {
        ...
        if ($aNotification['user_id'] == $aRow['user_id'] && !isset($aNotification['extra_users'])) {
            $sPhrase = _p('user_name_commented_on_gender_photo_title', [
                'user_name' => Phpfox::getService('notification')->getUsers($aNotification),
                'gender'    => Phpfox::getService('user')->gender($aRow['gender'], 1),
                'title'     => Phpfox::getLib('parse.output')->shorten($aRow['title'],
                    (Phpfox::isModule('notification') ? Phpfox::getParam('notification.total_notification_title_length') : $this->_iFallbackLength),
            ...
            $sPhrase = _p('user_name_commented_on_your_photo_title', [
                'user_name' => Phpfox::getService('notification')->getUsers($aNotification),
                'title'     => Phpfox::getLib('parse.output')->shorten($aRow['title'],
                    (Phpfox::isModule('notification') ? Phpfox::getParam('notification.total_notification_title_length') : $this->_iFallbackLength),
                    '...')
            ...
            $sPhrase = _p('user_name_commented_on_span_class_drop_data_user_full_name_s_span_photo_title', [
                'user_name' => Phpfox::getService('notification')->getUsers($aNotification),
                'full_name' => $aRow['full_name'],
                'title'     => Phpfox::getLib('parse.output')->shorten($aRow['title'],
                    (Phpfox::isModule('notification') ? Phpfox::getParam('notification.total_notification_title_length') : $this->_iFallbackLength),
    ...
    public function pendingApproval()
    {
        return [
            ...
            'link'   => Phpfox::getLib('url')->makeUrl('photo', ['view' => 'approval'])
        ];
    }
    ...
    public function getActivityFeed($aItem, $aCallback = null, $bIsChildItem = false)
    {
        ...
        $aRow = db()->select($sSelect)
            ...
            ->leftJoin(Phpfox::getT('photo_feed'), 'pfeed', 'pfeed.feed_id = ' . ($cacheFeedId ? $cacheFeedId : (int)$iFeedId))
        ...
        $sLink = ($iSponsorId > 0 ? Phpfox::getLib('url')->makeUrl('ad.sponsor', ['view' => $iSponsorId]) : Phpfox::permalink('photo', $aRow['photo_id'], $aRow['title']) . ('feed_' . (!$cacheFeedId ? $cacheFeedId : (int)$iFeedId)));
        $sFeedImageOnClick = '';
        ...
        if ($aRow['extra_photo_id'] > 0 && !$iSponsorId) {
            $total = db()->select('count(*)')
                ...
                ->where('pfeed.feed_id = ' . ($cacheFeedId ? $cacheFeedId : (isset($iFeedId) ? (int)$iFeedId : 0)) . ' AND p.album_id = ' . (int)$aRow['album_id'])
                ->executeField();
            ...
            foreach ($aPhotos as $aPhoto) {
                ...
                if ($indexing == 2 && $total > 3) {
                    $aListPhotos[] = '<a href="' . Phpfox::permalink('photo', $aPhoto['photo_id'],
                            $aPhoto['title']) . ('feed_' . ($cacheFeedId ? $cacheFeedId : (int)$iFeedId)) . '/" class="' . $sThickbox . ' photo_holder_image" rel="' . $aPhoto['photo_id'] . '" style="background-image: url(\'' . $sPhotoImage . '\')"><span>' . $sMore . '</span></a>';
                } else {
                    if ($photoMature) {
                        $aListPhotos[] = '<a href="#" class="no_ajax_link photo_holder_image photo-mature" onclick="tb_show(\'' . _p('warning') . '\', $.ajaxBox(\'photo.warning\', \'height=300&width=350&link=' . Phpfox::permalink('photo',
                                $aPhoto['photo_id'],
                                $aPhoto['title']) . ('feed_' . ($cacheFeedId ? $cacheFeedId : (int)$iFeedId)) . '/\')); return false;" style="background-image: url(\'' . $sPhotoImage . '\')"></a>';
                    } else {
                        $aListPhotos[] = '<a href="' . Phpfox::permalink('photo', $aPhoto['photo_id'],
                                $aPhoto['title']) . ('feed_' . ($cacheFeedId ? $cacheFeedId : (int)$iFeedId)) . '/" class="' . $sThickbox . ' photo_holder_image" rel="' . $aPhoto['photo_id'] . '" style="background-image: url(\'' . $sPhotoImage . '\')"></a>';
                    }
                }
            }
        }
        ...
        if (empty($aListPhotos)) {
            ...
            $aListPhotos = [
                strtr('<a href="{href}" {other}><img src="{img_src}"></a>', [
                    '{href}'    => $iSponsorId > 0 ? Phpfox::getLib('url')->makeUrl('ad.sponsor', ['view' => $iSponsorId]) : Phpfox::permalink('photo', $aRow['photo_id'], $aRow['title']) . ('feed_' . ($cacheFeedId ? $cacheFeedId : (int)$iFeedId)),
            ...
            $aListPhotos = array_merge([
                '<a href="' . Phpfox::permalink('photo', $aRow['photo_id'],
                    $aRow['title']) . ('feed_' . ($cacheFeedId ? $cacheFeedId : (int)$iFeedId)) . '/" ' . (empty($sFeedImageOnClick) ? ' class="' . $sThickbox . ' photo_holder_image" rel="' . $aRow['photo_id'] . '" ' : $sFeedImageOnClick . ' class="no_ajax_link photo_holder_image photo-mature"') . ' style="background-image: url(\'' . $sImageReturnUrl . '\')"></a>'
            ], $aListPhotos);
        }
        $sTypeId = 'photo';
        ...
        if ($total_feed_Tag) {
            ...
            $aReturn['friends_tagged'] = Phpfox::getService('feed')->getTaggedUsers($iItemId, $sTypeId, false, 1, 2);
        }
    ...
    public function getActivityFeedAlbum($aItem, $aCallback = null, $bIsChildItem = false)
    {
        ...
        foreach ($aPhotos as $aPhoto) {
            ...
            if ($indexing == 3 && $total > 4) {
                $aListPhotos[] = '<a href="' . Phpfox::permalink('photo', $aPhoto['photo_id'],
                        $aPhoto['title']) . 'album_' . $albumId . '" class="photo_holder_image" rel="' . $aPhoto['photo_id'] . '" style="background-image: url(\'' . $sPhotoImage . '\')"><span>' . $sMore . '</span></a>';
            } else {
                if ($photoMature) {
                    $aListPhotos[] = '<a href="#" class="no_ajax_link photo_holder_image photo-mature" onclick="tb_show(\'' . _p('warning') . '\', $.ajaxBox(\'photo.warning\', \'height=300&width=350&link=' . Phpfox::permalink('photo',
                            $aPhoto['photo_id'],
                            $aPhoto['title']) . 'album_' . $albumId . '/\')); return false;" style="background-image: url(\'' . $sPhotoImage . '\')"></a>';
                } else {
                    $aListPhotos[] = '<a href="' . Phpfox::permalink('photo', $aPhoto['photo_id'],
                            $aPhoto['title']) . 'album_' . $albumId . '" class="photo_holder_image" rel="' . $aPhoto['photo_id'] . '" style="background-image: url(\'' . $sPhotoImage . '\')"></a>';
                }
            }
        }
    ...
    public function getNotificationTag($aNotification)
    {
        ...
        if ($aRow['tagger_user_id'] == Phpfox::getUserId()) {
            $sPhrase = _p('you_tagged_yourself_in_your_photo_title', [
                'title' => Phpfox::getLib('parse.output')->shorten($aRow['title'],
                    (Phpfox::isModule('notification') ? Phpfox::getParam('notification.total_notification_title_length') : $this->_iFallbackLength),
                    '...')
            ]);
    ...
    public function getNotificationApproved($aNotification)
    {
        ...
        $sPhrase = _p('your_photo_title_has_been_approved', [
            'title' => Phpfox::getLib('parse.output')->shorten($aRow['title'],
