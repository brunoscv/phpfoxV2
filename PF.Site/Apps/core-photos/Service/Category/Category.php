    ...
    public function getCategory($iId)
    {
        ...
        if (!empty($aCategory['category_id'])) {
            ...
            foreach ($aLanguages as $aLanguage) {
                $aCategory['name_' . $aLanguage['language_id']] = _p($aCategory['name'], [], $aLanguage['language_id']);
            }
        }
        return $aCategory;
    }
        ...
        if ($iCnt) {
            ...
            foreach ($aPhotos as $iKey => $aPhoto) {
                $aPhotos[$iKey]['link'] = ($aCallback === null ? ($aPhoto['album_id'] ? $oUrl->makeUrl($aPhoto['user_name'],
                    ['photo', $aPhoto['album_url'], $aPhoto['title_url']]) : $oUrl->makeUrl($aPhoto['user_name'],
                    ['photo', 'view', $aPhoto['title_url']])) : $oUrl->makeUrl($aCallback['url_home'],
                    ['view', $aPhoto['title_url']]));
            }
        }
        ...
        foreach ($aCategories as $aCategory) {
            ...
            if ($bDropDown) {
                $sCategories .= '<option class="js_photo_category_' . $aCategory['category_id'] . '" value="' . $aCategory['category_id'] . '">' . ($this->_iCnt > 0 ? str_repeat('&nbsp;',
                            ($this->_iCnt * 2)) . ' ' : '') . Phpfox::getLib('locale')->convert(_p($aCategory['name'])) . '</option>';
                ...
                if ($bAnchor === true) {
                    $sCategories .= '<li><a href="' . Phpfox::getLib('url')->makeUrl('photo',
                            $mUrl) . '" class="js_photo_category" id="js_photo_category_' . $aCategory['category_id'] . '">' . Phpfox::getLib('locale')->convert(_p($aCategory['name'])) . '</a>' . $this->_get($aCategory['category_id'],
                            $bAnchor) . '</li>';
                } else {
                    $sCategories .= '<li><img src="' . Phpfox::getLib('template')->getStyle('image',
                            'misc/draggable.png') . '" alt="" /> <span class="js_photo_category" id="js_sortable_category_' . $aCategory['category_id'] . '">' . Phpfox::getLib('locale')->convert(_p($aCategory['name'])) . '</span>' . $this->_get($aCategory['category_id'],
                            $bAnchor) . '</li>';
                }
            }
        }
