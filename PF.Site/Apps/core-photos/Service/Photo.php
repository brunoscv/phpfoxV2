...
class Photo extends Phpfox_Service
{
    ...
    )
    {
        ...
        if ($iCnt) {
            ...
            $aPhotos = db()->select(Phpfox::getUserField() . ', p.*, pa.name AS album_url, pi.*, pa.profile_id, pa.cover_id')
                ->from(Phpfox::getT('photo'), 'p')
    ...
    public function getFeedLink($iItemId)
    {
        ...
        if (!$feed || ($feed && empty($feed['user_name']))) {
            $link = Phpfox::getLib('url')->permalink('photo', $iItemId, $title);
        } else {
    ...
    public function cropMaxWidth($sDes)
    {
        ...
        if ($iWidth < $width) {
            $bIsCropped = $oImage->createThumbnail($sDes, $sDes, $iWidth, $height, true, false);
            if ($bIsCropped !== false) {
                $oImage->addMark($sDes);
            }
        }
