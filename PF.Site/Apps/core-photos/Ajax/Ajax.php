    ...
    public function process()
    {
        ...
        if ($iNotCompleted === 0) {
            ...
            if (!Phpfox::getUserParam('photo.photo_must_be_approved') && !$this->get('is_cover_photo') && !$this->get('no_feed')) {
                ...
                if (Phpfox::isModule('feed')) {
                    if ($iTimeStamp && !empty($_SESSION['upload_photo_' . $iTimeStamp . '_' . $aPhoto['album_id']])) {
                        ...
                        if (isset($aVals['action']) && $aVals['action'] == 'upload_photo_via_share') {
                            ...
                            Phpfox::getService('photo.process')->notifyTaggedInFeed($aVals['status_info'], $aPhoto['photo_id'], $aPhoto['user_id'], $iFeedId, $aVals['tagged_friends'], $aVals['privacy'], (int)$this->get('parent_user_id', 0), $this->get('callback_module', ''));
                        }
                ...
                if ($coreVals && isset($coreVals['profile_user_id']) && !empty($coreVals['profile_user_id']) && $coreVals['profile_user_id'] != Phpfox::getUserId()) {
                    if (Phpfox::isModule('notification')) {
                        Phpfox::getService('notification.process')->add('photo_feed_profile', $aPhoto['photo_id'], $coreVals['profile_user_id']);
                    }
                    $link = Phpfox::getLib('url')->makeUrl(Phpfox::getUserBy('user_name'), ['feed' => $iFeedId]);
        ...
        if (!empty($aVals['link'])) {
            $this->call("$('#js_global_attach_value').val('');bCheckUrlForceAdd = false;checkMatch = [];$('#js_preview_link_attachment_custom_form_sub').remove();");
        }
    }
