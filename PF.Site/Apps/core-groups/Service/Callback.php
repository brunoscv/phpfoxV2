...
class Callback extends \Phpfox_Service
{
    ...
    public function getActivityFeed($aItem, $aCallback = null, $bIsChildItem = false)
    {
        ...
        if ((!\Phpfox::getService('groups')->isMember($aRow['page_id']) && in_array((int)$aRow['reg_method'], [1, 2]))) {
            return false;
        }
    ...
    public function getFeedComment($iId, $aVals)
    {
        ...
        $aCallback = [
            ...
            'subject' => ['{{ full_name }} wrote a comment on your group "{{ title }}".',
                ['full_name' => Phpfox::getUserBy('full_name'), 'title' => $aGroup['title']]],
            'message' => ['{{ full_name }} wrote a comment on your group "<a href="{{ link }}">{{ title }}</a>". To see the comment thread, follow the link below: <a href="{{ link }}">{{ link }}</a>',
    ...
    public function onVideoPublished($aVideo)
    {
        if ($aVideo && isset($aVideo['item_id'])) {
            ...
            $varPhraseTitle = 'full_name_posted_a_video_on_group_title';
            $varPhraseLink = 'full_name_posted_a_video_on_group_link';
            ...
            foreach ($aAdmins as $aAdmin) {
                ...
                if ($aGroup['user_id'] == $aAdmin['user_id']) { // is owner of group
                    $varPhraseTitle = 'full_name_posted_a_video_on_your_group_title';
                    $varPhraseLink = 'full_name_posted_a_video_on_your_group_link';
                }
                ...
                if (Phpfox::isModule('notification')) {
                    Phpfox::getService('notification.process')->add('groups_posted_video', $aVideo['video_id'], $aAdmin['user_id'], $aVideo['user_id'], $bForce);
                }
            }
        }
    }
    ...
    public function getToSponsorInfo($iId)
    {
        ...
        $aPage['link'] = Phpfox::permalink('groups', $aPage['item_id'], $aPage['title']);
