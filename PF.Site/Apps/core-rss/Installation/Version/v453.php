...
class v453
{
    ...
    private function _importForumToRssFeed()
    {
        ...
        if (!$iCntRss) {
            ...
            Lib::phrase()->addPhrase('rss_group_name_' . $iGroupId, 'Forum');
            Lib::phrase()->addPhrase('rss_title_' . $iGroupId, 'Latest Forum Topics');
            Lib::phrase()->addPhrase('rss_description_' . $iGroupId,
    ...
    private function _importBlogToRssFeed()
    {
        ...
        if (!$iCntRss) {
            $iGroupId = db()->insert(':rss_group', $aRssGroup);
            Lib::phrase()->addPhrase('rss_group_name_' . $iGroupId, 'Blogs');
            db()->update(':rss_group', ['name_var' => 'blog.rss_group_name_' . $iGroupId], 'group_id =' . $iGroupId);
            foreach ($aRssDatas as $key => $aRssData) {
                $rssTitle = 'blog_rss_title_' . $key;
                $rssDescription = 'blog_rss_description_' . $key;
                ...
                $aRssData['title_var'] = $rssTitle;
                $aRssData['description_var'] = $rssDescription;
                $aRssData['group_id'] = $iGroupId;
                db()->insert(':rss', $aRssData);
            }
        } else {
            // update phrase
            $aRss = db()->select('feed_id, title_var, feed_link')->from(':rss')->where(['module_id' => 'blog'])->executeRows();
            foreach ($aRss as $rss) {
                $aModules = explode('_', $rss['title_var']);
                $sModule = reset($aModules);
                if ($sModule == 'blog') {
                    continue;
                }

                foreach ($aRssDatas as $key => $aRssData) {
                    if ($aRssData['feed_link'] != $rss['feed_link']) {
                        continue;
                    }
                    $rssTitle = 'blog_rss_title_' . $key;
                    $rssDescription = 'blog_rss_description_' . $key;
    ...
    private function _importEventToRssFeed()
    {
        ...
        if (!$iCntRss) {
            $iGroupId = db()->insert(':rss_group', $aRssGroup);
            Lib::phrase()->addPhrase('rss_group_name_' . $iGroupId, 'Event');
            Lib::phrase()->addPhrase('rss_title_' . $iGroupId, 'Latest Events');
            Lib::phrase()->addPhrase('rss_description_' . $iGroupId, 'List of all the upcoming events.');
            ...
            db()->update(':rss', ['php_view_code' => '$aRows = Phpfox::getService(\'event\')->getForRssFeed();'],
                'module_id = \'event\' AND php_view_code = \'$aRows = Event_Service_Event::instance()->getForRssFeed();\'');
        }
    }
}
