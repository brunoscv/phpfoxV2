...
class Process extends Phpfox_Service
{
    ...
    public function addGift(array $aVals)
    {
        ...
        $this->cache()->remove('egift_total_0');
        if(!empty($aVals['category']))
        {
            $this->cache()->remove('egift_total_' .(int)$aVals['category']);
        }
    ...
    public function deleteGift($iId)
    {
        ...
        $this->cache()->remove('egift_item');
        $this->cache()->remove('egift_total_0');
        if(!empty($aGift['category_id']))
        {
            $this->cache()->remove('egift_total_' .(int)$aGift['category_id']);
        }
        return true;
    }
