...
class OdnoklassnikiController extends Phpfox_Component
{
    public function process()
    {
        ...
        $adapterId = $this->request()->get('adapter') ? $this->request()->get('adapter') : 'Odnoklassniki';
        ...
        if ($adapter["adapter"] == 'Odnoklassniki') {
            $config = [
                'callback' => $indexUrl.'odnoklassniki/',
                'providers' => [
                    "{$adapter["adapter"]}" => [
                        'enabled' => true,
