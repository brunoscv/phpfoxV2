...
class Api extends ApiServiceBase
{
    ...
    /**
     * add comment for a specific item
     * @return array|bool
     * @throws \Exception
     */
    public function post()
    {
        ...
        if (isset($aVals['type']) && $aVals['type'] != 'app' && Phpfox::hasCallback($aVals['type'], 'getAjaxCommentVar')) {
        ...
        if (!Phpfox::getUserParam('comment.can_post_comments')) {
            return $this->error(_p('your_user_group_is_not_allowed_to_add_comments'));
        }
        ...
        if (($iFlood = Phpfox::getUserParam('comment.comment_post_flood_control')) !== 0) {
            $aFlood = [
                ...
                'params' => [
                    ...
                    'condition'  => 'type_id = \'' . db()->escape($aVals['type']) . '\' AND user_id = ' . Phpfox::getUserId(), // Database WHERE query
            ...
            if (Phpfox::getLib('spam')->check($aFlood)) {
                return $this->error(_p('posting_a_comment_a_little_too_soon_total_time', ['total_time' => Phpfox::getLib('spam')->getWaitTime()]));
            }
        }
