	...
	{* <ul class="nav nav-tabs nav-justified">
		...
		{if count($aGroups)}
			{foreach from=$aGroups name=groups item=aGroup}
				<li class="{if $phpfox.iteration.groups == count($aGroups) && Phpfox::isAppActive('Core_Music') && !Phpfox::getUserParam('music.can_upload_music_public')} last{/if}">
...
<form method="post" class="form" action="{url link='user.profile'}"{if !$bIsEdit} onsubmit="{plugin call='user.template_controller_profile_form_onsubmit'} $Core.processing(); $(this).ajaxCall('custom.updateFields'); return false;"{/if}>
	...
	<div class="js_custom_groups js_custom_group_basic">
		<div class="profile-edit-headline">
			<span class="content">
				<span class="ico ico-text-file"></span>
				{_p var='basic_information'}
			</span>
		</div>
    ...
    {foreach from=$aGroups item=aGroup}
        <div class="js_custom_groups js_custom_group_basic">
            <div class="profile-edit-headline">
            <span class="content">
                <span class="ico {$aGroup.ico}"></span>
                {$aGroup.phrase_var_name}
            </span>
            </div>
            {foreach from=$aSettings item=aSetting}
                {if $aGroup.group_id == $aSetting.group_id}
                <div class="form-group js_core_init_selectize_form_group js_custom_groups js_custom_group_{$aSetting.group_id}">
                    <label class="table_left" {if $aSetting.is_required} class="required}"{/if}>
    ...
    {if empty($aSettings)}
        <p class="help-block">
            {if Phpfox::getUserParam('custom.can_add_custom_fields')}
                ...
                <ul class="action">
                    <li><a target="_blank" class="no_ajax" href="{url link='admincp.custom.add'}">{_p var='add_a_new_custom_field'}</a></li>
                </ul>
            {/if}
        </p>
    {/if}
