...
{if $aUser.user_id != Phpfox::getUserId() && !$bIsPage}
    {if $iMutualTotal > 0}
        <ul class="user_tooltip_mutual mutual-friends-list">
            ...
            {if $iRemainFriends > 0}
                ...
                <a href="#" class="user_viewmore" onclick="$Core.box('friend.getMutualFriends', 450, 'user_id={$aUser.user_id}');return false;">{if $iRemainFriends == 1}{_p var='1_other'}{else}{_p var='total_others' total=$iRemainFriends}{/if}</a>
            {/if}
        </ul>
    {/if}
    <div class="user_tooltip_action_user">
        ...
        {/if}
        <ul>
            {if !$bLoginAsPage && Phpfox::isUser() && Phpfox::isModule('friend') && Phpfox::getUserParam('friend.can_add_friends') && empty($aUser.is_friend)}
