...
<div data-feed-id="{$aFeed.feed_id}" data-feed-update="{$aFeed.time_update}" class="feed-stream-content
	{if (isset($sponsor) && $sponsor) || (isset($aFeed.sponsored_feed) && $aFeed.sponsored_feed)}sponsor{/if} _app_{$aFeed.type_id} {if isset($aFeed.custom_class)}{$aFeed.custom_class}{/if} js_parent_feed_entry js_user_feed" id="js_item_feed_{$aFeed.feed_id}" {if !empty($bForceFlavor)}data-force-flavor="material"{/if}>
