                                ...
                                <div class="drop_data_content">
                                    <div class="drop_data_user">
                                        <div class="drop_data_action">
                                            <div class="js_drop_data_button" id="drop_down_{$aFriend.request_id}">
                                                <ul class="table_clear_button inline">
                                                    <li><a type="button" name="" value="" class="button btn-icon btn btn-primary btn-sm " onclick="$(this).parents('.drop_data_action').find('.js_drop_data_add').show(); {if $aFriend.relation_data_id > 0} $.ajaxCall('custom.processRelationship', 'relation_data_id={$aFriend.relation_data_id}&amp;type=accept&amp;request_id={$aFriend.request_id}'); {else} $.ajaxCall('friend.processRequest', 'type=yes&amp;user_id={$aFriend.user_id}&amp;request_id={$aFriend.request_id}&amp;manage_all_request=true'); {/if}" ><span class="ico ico-check"></span><span class="item-text">{_p var='confirm'}</span></a></li>
                                                    <li><a type="button" name="" value="" class="btn-delete btn-icon button button_off btn btn-default btn-sm " onclick="$(this).parents('.drop_data_action').find('.js_drop_data_add').show(); {if $aFriend.relation_data_id > 0} $.ajaxCall('custom.processRelationship', 'relation_data_id={$aFriend.relation_data_id}&amp;type=deny&amp;request_id={$aFriend.request_id}'); {else} $.ajaxCall('friend.processRequest', 'type=no&amp;user_id={$aFriend.user_id}&amp;request_id={$aFriend.request_id}&amp;manage_all_request=true'); {/if}" ><span class="ico ico-close" style="display: none;"></span><span class="item-text">{_p var='delete_request'}</span></a></li>
                                                </ul>
                                                <div class="clear"></div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="dropdown extra_info_middot dropup" style="display:none;">
                                            <a class="btn btn-icon btn-default btn-sm" type="button" data-toggle="dropdown"><span class="ico ico-check"></span>{_p var='friend'}
                                                <span class="caret"></span></a>
                                            <ul class="dropdown-menu dropdown-menu-right">
                                                <li><a role="button" onclick="$Core.composeMessage({l}user_id: {$aFriend.user_id}{r}); return false;"><span class="ico ico-comment-o"></span><span class="item-text">{_p var='send_message'}</span></a></li>
                                                <li><a role="button" onclick="$Core.jsConfirm({l}{r}, function(){l}$.ajaxCall('friend.delete', 'friend_user_id={$aFriend.user_id}&reload=1');{r}, function(){l}{r}); return false;"><span class="ico ico-user1-del-o"></span><span class="item-text">{_p var='unfriend'}</span></a></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
