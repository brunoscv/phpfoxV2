...
<div id="js_register_step2">
	...
	<div class="form-group">
		{required}<label class="">{_p var='birthday'}</label>
	...
	<div class="form-group">
		{required}<label for="gender" class="">{_p var='i_am'}</label>
		{select_gender}
	</div>
	{/if}
	...
	<div class="form-group js_core_init_selectize_form_group selectize-mcustomscroll">
		{required}<label for="country_iso" class="">{_p var='location'}:</label>
		{select_location}
...
{if Phpfox::getParam('user.force_user_to_upload_on_sign_up')}
	<div class="form-group register-upload-photo">
		{required}<label class="">{_p var='profile_image'}</label>
