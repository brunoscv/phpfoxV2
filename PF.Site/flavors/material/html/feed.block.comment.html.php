				...
				<div class="comment-mini-content-commands">
					<div class="button-like-share-block {if isset($aFeed.total_action)}comment-has-{$aFeed.total_action}-actions{/if}">
						...
						<div class="feed-like-link">
							...
							<span class="counter" onclick="return $Core.box('like.browse', 450, 'type_id={if isset($aFeed.like_type_id)}{$aFeed.like_type_id}{else}{$aFeed.type_id}{/if}&amp;item_id={$aFeed.item_id}');">{if !empty($aFeed.feed_total_like)}{$aFeed.feed_total_like|number_format}{/if}</span>
						</div>
						{/if}
                        {if (!isset($sFeedType) ||  $sFeedType != 'mini') && $aFeed.can_comment}
                            <div class="feed-comment-link">
                                <a href="#" title="{_p var='comment'}" onclick="$('#js_feed_comment_form_textarea_{$aFeed.feed_id}').focus();return false;"><span class="ico ico-comment-o"></span></a>
                                <span class="counter">{if !empty($aFeed.total_comment)}{$aFeed.total_comment|number_format}{/if}</span>
                            </div>
                        {/if}
