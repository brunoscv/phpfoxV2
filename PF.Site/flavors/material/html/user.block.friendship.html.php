...
{if empty($no_button) && !$bLoginAsPage && Phpfox::isUser() && Phpfox::isModule('friend') && Phpfox::getUserParam('friend.can_add_friends')}
    ...
    <div class="btn-addfriend">
        <a class="btn btn-primary btn-gradient s-5 btn-round" href="#" onclick="return $Core.addAsFriend('{$user_id}'{if !empty($bInSuggestionsBlock)},true{/if});" title="{_p var='add_to_friends'}">
            {if $type == 'string'}
            ...
            {else}
                ...
                {foreach from=$mutual_list key=iKey item=aMutualFriend}
                    <li class="user_profile_link_span" id="js_user_name_link_{$aMutualFriend.user_name}">
                        <a href="{url link=$aMutualFriend.user_name}">{$aMutualFriend.full_name|clean}</a>
                    </li>
                {/foreach}
                {if $mutual_remain > 0}
                    {_p var='and'}
