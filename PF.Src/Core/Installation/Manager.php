...
class Manager
{
    ...
    /**
     * @param array $param
     * @param bool $beforeInstall
     * @return string
     */
    public function install($param, $beforeInstall = true)
    {
        // install app when install site
        ...
        // install app in admincp
        if ($beforeInstall) {
            ($sPlugin = \Phpfox_Plugin::get('on_before_install_app'))? eval($sPlugin): false;

            $this->onBeforeInstall($param);

            return Phpfox::getLib('url')->makeUrl('admincp.store.ftp', ['param' => base64_encode(json_encode($param))]);
        } else {
            $returnUrl = $this->onInstall($param);
            $this->onAfterInstall($param);

            ($sPlugin = \Phpfox_Plugin::get('on_after_install_app'))? eval($sPlugin): false;
    ...
    public function copyFile($param)
    {
        ...
        if (!empty($result['files'])) {
            // backup before replace file
            if ($this->_is_upgrade) {
                $this->backupApp($param);
            }
     ...
     * @param $param
     */
    public function backupApp($param)
    {
        ...
        $appPath = realpath(PHPFOX_DIR_SITE) . PHPFOX_DS . 'Apps';
        $backupPath = PHPFOX_DIR_FILE . 'static' . PHPFOX_DS . 'backupApps';
        $fileList = $this->scanFiles($appPath . PHPFOX_DS . $name);
        $this->managerControl->setFile($fileList)
            ->setFromPath($appPath)
            ->setToPath($backupPath)
            ->run();
    }
     ...
     * @param $param
     */
    public function restoreApp($param)
    {
        if (empty($param['productName'])) {
            throw new \InvalidArgumentException(_p('Missing params [name]'));
        }
    ...
    {
        $this->managerControl->deleteSingFolder(PHPFOX_DIR_FILE . 'static' . PHPFOX_DS . 'backupApps');
    }
    ...
    public function runInstallForApp($param)
    {
        ...
        if ($this->_is_upgrade) {
            $is_upgrade = true;
        }else if(isset($param['is_upgrade']) && $param['is_upgrade']){
			$is_upgrade = true;
		}
        ...
        return $this->callRunInstallForApp($productName, $sAppDirName, $is_upgrade);
	 ...
	 * @return string|null
	 */
    public function dryInstall($param){
    ...
    private function onAfterInstall($param)
    {
        ...
        // clear all cache belong to apps
        Phpfox::getLib('cache')->remove('lib_all_apps');
        Phpfox::getLib('cache')->remove('lib_aAppDirs');
        Phpfox::getLib('cache')->remove('app_is_active');
        Phpfox::getLib('cache')->remove('apps_for_manage');
        Phpfox::getLib('cache')->removeGroup('admincp');
    }
...
//            unlink($filename);
    ...
    /**
     * @param string $newDirList
     * @param string $oldDirList
     *
    ...
    public function callRunInstallForApp(
        ...
        $base = realpath(PHPFOX_DIR_SITE) . PHPFOX_DS . 'Apps' . PHPFOX_DS . $sAppDirName . PHPFOX_DS;;
        $baseTemp = PHPFOX_DIR_FILE . 'static' . PHPFOX_DS . 'phpfox-app-' . $productName . PHPFOX_DS;
        ...
        return Phpfox::getLib('url')->makeUrl('admincp.app', ['id' => $Object->id, 'verify' => $is_upgrade]);
    }
}