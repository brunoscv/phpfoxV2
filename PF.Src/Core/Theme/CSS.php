...
class CSS extends \Core\Model
{
    ...
    public function set($content, $vars = null, $more_content = '', $themeName = 'default')
    {
        ...
        // check and compile LESS to CSS with node js on phpFox server
        if (!defined('PHPFOX_SKIP_COMPILE_REMOTE') && $this->ping_remote()) {
        ...
        $sCacheFolder = PHPFOX_DIR_FILE . 'cache' . PHPFOX_DS;
        if (!is_dir($sCacheFolder)) {
            mkdir($sCacheFolder);
        }

        // enable cache options
        $less->setOption('cache_dir', $sCacheFolder . 'less');
        $parsed = null;
        try {
            ...
            Phpfox::getLib('assets')->putAssetContents(str_replace(PHPFOX_PARENT_DIR, '', $cssOutputFileName),$parsed);
    ...
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'http://sys-compile-css.phpfox.com/?ping=1');
    ...
    public function compile_remote($root_file, $target)
    {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'http://sys-compile-css.phpfox.com/');
        ...
        if ($content and substr($content, 0, 4) == 'http') {
            ...
            curl_close($ch);
            file_put_contents($target, $css,LOCK_EX);
            Phpfox::getLib('assets')->putAssetContents(str_replace(PHPFOX_PARENT_DIR, '', $target), $css);
        }
        return true;
    }
    ...
    public function buildFile($fileName, $locationBuild = 'module', $themeName = 'default')
    {
        ...
        //build
        $lessContent = file_get_contents($suffixPath . $fileName);
        ...
        Phpfox::getLib('assets')->putAssetContents(str_replace(PHPFOX_PARENT_DIR, '', $path . '.css'));
    }
    ...
    public function getApp()
    {
        ...
        foreach ($Apps->all() as $App) {
            ...
            if (file_exists($assets . 'main.less')) {
                $app_less_contain .= "/*Begin ' . $App->name . '*/\r\n" . file_get_contents($assets . 'main.less') . '/*End ' . $App->name . "*/\r\n";
            }
        }
